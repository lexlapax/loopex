...
| Close | An indexed evidence-page scaffold at the tested SHA; `bash scripts/check.sh` under the floor pair (replace `NAME` with the milestone name in `env -u MIX_BUILD_PATH MIX_BUILD_ROOT="/absolute/retained-work/NAME-otp27-build" mise exec erlang@27.3.4 elixir@1.18.5-otp-27 -- bash scripts/check.sh`); `bash scripts/check-release.sh` once on the current pair, with its fresh-source lane staging the tested archive under the [canonical archive-extraction rule](milestones-technical.md#technical-milestones-archive-extraction) and retaining outside the extraction the exact NUL-delimited output of `bash "$tree/scripts/source-archive-manifest.sh" "$tree"` | Release check: 149–163 s on Linux, 12 tests |
