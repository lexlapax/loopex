...
Elixir 1.20.3 / OTP 29.0.5, Homebrew) and the Linux host serenity (x86_64,
...
| Close | An indexed evidence-page scaffold at the tested SHA; `bash scripts/check.sh` under the floor pair (replace `NAME` with the milestone name in `env -u MIX_BUILD_PATH MIX_BUILD_ROOT="/absolute/retained-work/NAME-otp27-build" mise exec erlang@27.3.4 elixir@1.18.5-otp-27 -- bash scripts/check.sh`); `bash scripts/check-release.sh` once on the current pair, with its fresh-source lane staging the tested archive under the [canonical archive-extraction rule](milestones-technical.md#technical-milestones-archive-extraction) and retaining outside the extraction the exact NUL-delimited output of `bash "$tree/scripts/source-archive-manifest.sh" "$tree"` | Release check: 149–163 s on Linux, 12 tests, at M4; M5's larger lane set is measured in [its closure evidence](../evidence/M5-closure-runs.md) |
| Release | Before tag creation, on the administrative closure SHA, the four rows of the milestone guide's [Tags table](milestones-technical.md#technical-milestones-release), which owns the exact commands: confine the administrative commit's complete retained patch to [its five paths and allowed regions](milestones-technical.md#technical-milestones-confinement), including the byte-for-byte reconstruction of `docs/plans/README.md` (its `<NAME>` register row and marked Current Status block) and the root `README.md` (its marked status block); run `check.sh --docs`; run the final semantic operator/developer documentation gate; and stage a fresh `git archive` extraction under the same [canonical archive-extraction rule](milestones-technical.md#technical-milestones-archive-extraction), run `bash "$tree/scripts/source-archive-manifest.sh" "$tree" >"$retained_manifest"` with the output outside the extraction, and compare the retained bytes with the tested manifest after the documented exclusions | The documentation-scoped check alone: 12–13 s on the Mac at this revision, warm build, plus the confinement review, semantic review and manifest comparison. No suite and no release check |

These Close and Release rows apply from M5 onward. M4 and its `v0.1.0` tag
remain historical records governed by the procedure they name.

The tested commit itself owns `In progress` to `In review` and both marked
status blocks; its administrative direct child owns only `In review` to
`Closed`. The tested candidate's `Proved` rows map completed implementation to these
proof obligations. Its predeclared `Pending` scaffold fields hold the results
or identities produced when the obligations are run against that commit; they
are filled only by the administrative direct child.
...
Evidence retention: the tested candidate carries an indexed scaffold under
...
retained-output reference, and SHA-256 digest. The fresh-source lane retains
the tested archive manifest's exact NUL-delimited bytes under a separate
reference and digest. It also records the independent
...
identities, not output. Release proofs run later against the
administrative SHA. Their complete outputs stay outside the repository, and
the annotated tag records their results, retained-output references, and
SHA-256 digests when the tag is created. Neither lane needs a third commit.
...
The procedure that reproduced it on serenity, and that a change to an
operating-system process boundary is read against: pin the test VM and eight busy-loop
