...
| Release | Before tag creation: confine the administrative commit's complete retained patch to its five paths and each path's exact allowed region, reconstructing `docs/plans/README.md` from tested bytes by replacing its M5 register row and marked Current Status block, and the root `README.md` by replacing only its marked status block; run `check.sh --docs` and the final semantic operator/developer documentation gate on that SHA; stage a fresh `git archive` extraction under the same [canonical archive-extraction rule](milestones-technical.md#technical-milestones-archive-extraction) and run `bash "$tree/scripts/source-archive-manifest.sh" "$tree" >"$retained_manifest"` with the output outside the extraction; retain its exact bytes; require each complete unexcluded `(kind, mode, path)` projection to match its commit’s independent Git-tree projection; require the tested and administrative projections to match each other; reject malformed or duplicate records; remove `docs` and its descendants plus exact root `README.md` and `SOURCE_IDENTITY`; compare every remaining complete tuple with the retained tested manifest; validate both archive identities | The documentation-scoped check alone: 12–13 s on the Mac at this revision, warm build, plus the confinement review, semantic review and manifest comparison. No suite and no release check |
