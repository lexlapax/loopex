...
| Daemon wire protocol | the `loopex.experimental/2` generation over the daemon's Unix-domain socket: generation 1's records plus `session.list`, `daemon.status`, `session.acquire_control`, `session.release_control`, the `writer_epoch` on existing-session mutations, and the `daemon.stopping` and `daemon.notice` records | 2, public protocol semantics | Experimental, new in M5; exact generation agreement only |
| Live CLI and daemon commands | `loopex daemon`, `loopex daemon prepare-index`, the `--daemon` forms of `run`, `resume` and `sessions`, `loopex attach`, their exit statuses and the compact JSON of live queries | not yet a listed surface | Unstable, new in M5 |
