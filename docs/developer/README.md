...
| [The daemon](daemon.md#concept) · [technical](daemon-technical.md#technical-depth) | The host that keeps a root's sessions alive between processes: why it is a host and not a second loop, why controller authority stays out of the journal, how nothing admitted is forgotten, durable-first delivery, the lifecycle order and the read-once credential; then its processes, startup order, stop clocks, one connection's life, output and progress bounds, succession and the tests behind each claim. |
...
| [Verification](verification.md#concept) · [technical](verification-technical.md#technical-depth) | The rule book for checking work now that the milestone gates are retired: three stages with one question each, how a changed boundary selects its checks, the rules that keep the checks honest, and the measured plan for making the push check fast; then what each stage runs, where each boundary's checks live, and the per-application and per-test measurements behind the plan. |
| [Milestones](milestones.md#concept) · [technical](milestones-technical.md#technical-depth) | How to plan, run and close a milestone under the post-M4 structure: the plan pair, small reviewed merges, the closure packet and the release decision; then the exact plan sections, register row, branch and worktree conventions, the closure commands and the tag. |
| [Compatibility surfaces](compatibility-surfaces.md#concept) | M2, M3, M4 and M5 surfaces with their experimental labels, the app server's exact-generation rule, interaction-record old-reader refusal and rollback, and why none is labelled or frozen. |
