...
| [App server protocol](app-server-protocol.md#concept) · [technical](app-server-protocol-technical.md#technical-depth) | The normative wire reference for the foreground generation `loopex.experimental/1`: why one contract rather than a second loop, then its sixteen methods, seven record families, fifteen error codes, encodings, limits and evidence. |
...
| [Compatibility surfaces](compatibility-surfaces.md#concept) | The surfaces M2 through M5 expose to an embedder or client, their experimental labels, the app server's exact-generation rule, interaction-record old-reader refusal and rollback, and why none is labelled or frozen. |
