...
| Close | Did the milestone deliver its outcomes? | `check.sh` once under the floor toolchain pair (the current pair is already proved by CI on every change), `bash scripts/check-release.sh` once, the outcome-to-evidence map in the plan, an independent review of the candidate — all of it from the **tested implementation SHA**, once; the **administrative closure SHA** that records the decision re-runs nothing | The maintainer, who closes it or does not |
| Release | Is the published source the closed source? | The closure evidence, reused when the source is unchanged; the tag on the **administrative closure SHA**, whose diff from the tested SHA is verified to be confined to the four paths the milestone guide fixes (all under `docs/`), plus the two documentation-scoped re-proofs that confinement makes sufficient — `check.sh --docs` on the tagged SHA, and a recomputed archive manifest matching the tested SHA's for every entry outside `docs/`. No suite and no release check run twice | The maintainer's separate release decision |
