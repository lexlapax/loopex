...
provenance. Archived files are non-normative: none carries a lifecycle state, a
register row, or any authority, and current authority follows the root
`AGENTS.md`. Do not bulk-read them or use them as implementation guidance.
Consult them for an explicit provenance or historical-comparison task.

Git history preserves byte-identical snapshots, so copies of current files do
not belong here. Keep historical seed material immutable. The M4 draft that
lived here until its opening now lives in `docs/plans/`; its archived revisions
stay in Git history.
