...
not admit a command. For an existing session, the daemon admits a mutation
only when the sending connection is the current holder, its supplied writer
epoch matches, the lease is held and unexpired, and the attachment has command
capability. The daemon checks those facts together before core admission and
serializes that handoff with lease changes; an observer cannot reuse an epoch
it learned from a result or status. Generation-2 `session.create` is the one
exception because no session lease exists yet: it creates an uncontrolled
session, after which the client attaches and explicitly acquires control before
its first session command. For a known dormant session, a generation-2 client
may acquire control by session ID before `session.resume`, then resume with
the granted epoch and attach. Daemon-internal startup recovery uses state-root
authority rather than a client lease. A
...
Generation-1 daemon connections have no wire epoch. A successful attachment
occupies an exclusive session slot and receives an internal lease tied to that
connection. A generation-1 `session.resume` obtains that exclusive internal
lease before forwarding the resume; its subsequent attachment uses the same
lease. Its commands undergo the same holder, held-state and expiry check;
another connection of either generation cannot attach concurrently or take a
held lease. The M4 foreground server's single-connection behavior is unchanged.
If an idle generation-1 connection reaches lease expiry while still attached,
the daemon detaches and fences it before another connection can take control
or attach.
...
release on orderly disconnect. After a daemon restart, the old connection is
invalidated durably only after the previous root writer is proved gone; an
uncertain writer blocks startup.
