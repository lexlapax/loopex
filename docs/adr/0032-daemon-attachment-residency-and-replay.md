...
- **Supersedes:** 0023, only the clause that limits a process to one active attachment; the rest of ADR 0023 stays in force and its generation-1 wire stays served
...
JSONL protocol unchanged over a Unix-domain socket: same framing, same
initialize handshake, same records. Generation `loopex.experimental/2` adds
the daemon methods, `session.list`, `session.stop`, `daemon.status` and the
lease methods ADR 0033 names, and lifts the one-attachment-per-process rule to
a per-session and per-daemon residency limit; generation 1 clients keep being
served exactly as the foreground server serves them. The socket path is the
...
session, ten minutes of idle time before eviction, and the ADR 0023 frame
ceiling unchanged.
...
The public protocol remains experimental with exact-generation agreement and
no mixed-generation promise.

Rollback is the M4 foreground server on the same store: it carries no
