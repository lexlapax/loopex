...
| 7 | `apps/loopex/test/project_resource_trust_test.exs` / default | 8 | passed: `discovery resolves one canonical resource under declared path and size limits and retains the future shape total`; `the operator is shown every resolved path its provenance and the manifest digest`; `an explicit trust decision binds workspace revision manifest and digests`; `a changed workspace revision manifest or content invalidates the decision`; `a headless run without a matching positive decision stages no project block journals a declined receipt and still runs`; `an ordinary workspace read stays a policy governed tool effect and is never context staging`; `an admitted project block changes no tool set policy decision bound or grant`; `project manifest and trust labels are closed bounded safe text before retention` |
...
<a id="amendment-7"></a>
## Amendment 7 — Prove the Operator Display Where the Operator Is

**Acceptance: OUTSTANDING.** This section advances the generation and rebinds the amended artifact named below. The prior Acceptance row and `In review` lifecycle state remain at this proposal. Its immediate one-parent child must rebind Acceptance to this exact revision and add one new amendment-7-specific disposition anchor without changing any other byte.

Outcome 7's locked selector named a case, `the real operator decision path displays resolved path provenance trust and both digests`,
whose body drives the command's discovery and decision modules and asserts
what the terminal displays. That is command behaviour: the modules belong to
the command application, and the bound runner refuses any sibling application
outside the owning application's declared closure, so the case can pass only
under an umbrella test run and never through the gate. It was written against
the umbrella run and had not been proved through the runner before the probe
turned green.

This amendment moves that case, body unchanged, into the command selector's
file, where the runner's closure holds every module it calls. Outcome 7 keeps
its remaining eight named cases and its minimum falls to eight; the command
selector gains the case and its minimum rises to twenty-two. No case body,
claim, or product byte changes.

**Maintainer disposition.** The maintainer, on 2026-09-02, authorized this
amendment to bypass the exact-SHA independent review the ordinary transaction
requires, as a recorded override: the change moves one case between two
locked selectors without altering what it proves, and the review cost
exceeded the change. The rebind that follows records that override as its
disposition.
