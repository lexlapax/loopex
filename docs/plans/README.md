...
**Revision status:** Closed milestone product baseline; active milestone `M2` is in progress; no next candidate is recorded.

| Field | Value |
| --- | --- |
| Integrated phase | Closed milestone product baseline |
| Last closed product checkpoint | `M1` — 2026-08-23 |
| Blockers | None; `M2` is in progress against its locked gate |
...
| Next transition | Turn the locked gate green, then move `M2` to In review |
...
| `M2` | In progress | [concept](M2.md) | [technical depth](M2-technical.md) | [gate](M2-gate.md) |
