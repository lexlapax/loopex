...
Core change 1 also adds `@doc false Runtime.command_for_daemon/2` and its
matching detailed coordinator call while leaving released
`Runtime.command/2` unchanged. Before a coordinator call, a dead exact
coordinator returns `session_unavailable`; an attachment already removed by
succession returns its identity for an exact join to the daemon's local
succession row. A successful route returns an opaque token bound to the
attachment, coordinator and owner generation. Every matching
`superseded_owner` from the coordinator's phase, owner, superseded or nested
`Control.current_owner/3` check occurs before `commit_command/2` and is positive
`superseded_before_admission`; any other lost reply after routing is
`admission_unknown`, even if Control has moved to a later generation. Loss of
Control or a missing succession join is `runtime_unavailable` and daemon-fatal.
Before rendering any real accepted or refused result, the relay passes its route
token to `Runtime.classify_daemon_result/2`; Control orders that read-only
classification against the succession cut. A result classified before the cut
— because its exact attachment and incarnation remain live, including through
prepared succession — may render first, while one classified after a
non-prepared route removal joins the exact holder notice
and renders only after `detached`. The helper derives the answer from the token
and current state, retains no tombstone, and exact-Control loss is fatal.
Targeted replacement uses no route disposition: the registry closes the exact
target's local mutation gate, settles or reaps every predecessor mutation
origin, and only then calls core and publishes the new handle. One registry CAS
chooses local publication or succession ownership. Succession winning first at
any pre-finalization core phase converts the target's borrowed charge and reserve
to one succession row, publishes no replacement handle, and returns the pending
attach's `attachment_conflict` after `detached`; publication winning first
transfers both to the new installed attachment before ordinary succession.
...
by attachment identity even if holder `DOWN` races, enqueues one uncorrelated
`detached` record carrying the session ID and last completely emitted cursor,
and leaves that connection open. A pending or installed attachment reserves
`succession_delivery_reserve_bytes` locally and in the 512 MiB aggregate: the
maximal encoded `detached` plus one maximal correlated succession-reply slot,
derived across every mutation disposition and pending-attach
`attachment_conflict`.
The reduced ordinary allowance remains while the attachment is pending or
installed or its local succession row remains.
After the notice is completely emitted, at most 32 retained old-attachment
mutation origins reuse the reply slot serially in admission order; reattach
waits until their replies are emitted or the connection is reaped. The connection
dispatcher pauses reading and promoting post-cut requests behind that row, then
resumes them in wire order after it clears, so a query, mutation or reattach
cannot consume or starve the slot. A query, read, transfer or delivery result
admitted before the cut remains ordinary output and may independently invoke
the existing overflow close. Every encoded-byte
admission preserves `charged_encoded_bytes + pending_admission_bytes +
unused_succession_reserves <= 512 MiB`. Replacement transfers the target's
local and aggregate reserve after its predecessor barrier, so it stays
net-zero at full attachment and aggregate capacity. The total stays 4 MiB, and
buffer saturation cannot drop the succession delivery or close the connection.
This is the fourth generation-2 `detached` occasion. A controller's lease,
epoch and deadline remain unchanged; the
...
- **the extracted-build witness separately compares a canonical manifest**,
  ...
  caller umask under the canonical extraction rule below and of
  platform-specific link or directory modes. `path` is relative to the
...
check stages the archive with `git archive` from a known commit and creates and
populates its extraction in a subshell that sets `umask 022` before either
operation, as the [canonical archive-extraction rule](../developer/milestones-technical.md#technical-milestones-archive-extraction)
requires. The witness invokes that subshell from hostile caller `umask 0777`
and requires the extracted manifest's complete unexcluded `(kind, mode, path)`
projection to match the commit's independently derived Git-tree projection,
including every executable bit and directory mode. It then obtains the same
ordered projection from the tested and administrative archives before any
content exclusion. It retains the
...
The source archive is staged from the exact committed candidate with
`git archive` and extracted outside the checkout through that same canonical
scoped-`umask 022` rule, with the retained proof launched from caller
`umask 0777`. Before the build, the fresh-source lane runs
...
extraction of the administrative SHA uses the same canonical scoped-`umask
022` rule from caller `umask 0777`, then runs its own copy of
...
| Core succession invalidates an installed attachment | A received `detached` record is attachment loss, never evidence that the session is dormant and never settlement of a pending request. The connection remains open. Capacity-reserved correlated mutation replies follow it serially; a real admission stays real, positive pre-commit succession is `control_not_held`, and a post-route lost reply is `admission_unknown`. Reattach waits until every predecessor mutation origin is terminal and its reply emitted or the connection is reaped. An observer then reattaches at the retained cursor without activating; a controller retains its lease and epoch, with its deadline changed only by an independently admitted or unknown mutation, then reattaches and inspects. Neither role sends `session.resume`, releases, reacquires or waits for expiry solely because succession invalidated the old handle. A controller-issued resume that caused the cut receives `detached` before its one real resume result and then reattaches. A separate transport loss follows the ordinary EOF recovery rows. |
| A temporary coordinator dies after attach | The normal live-resume preflight is bounded `session.inspect`: exact `session_unavailable` admits nothing. If death instead races a later mutation, death proved before the daemon-only coordinator call returns the same correlated `session_unavailable`; a lost reply after a successful route returns `admission_unknown`, advances no next step and is resolved only by re-presenting the exact method, command ID and canonical input. The connection stays open for a bounded release attempt. Once every retained command disposition is resolved or reported unresolved, restart the daemon and run resume so the new lifetime treats the session as dormant and creates one coordinator. |
...
- **the transport monitor**, so a connection that dies closes its socket and
  ...
  same `DOWN`;
- **the buffer-control interface**, so backpressure, the
  `succession_delivery_reserve_bytes` reservation inside the existing 4 MiB total and
  every bounded write go through one place rather than each caller reaching
...
- **At most one unresolved mutation ticket per session**, which is the
  ...
  **lease owner** is the only process that makes ticket calls for a session,
  so it is where the sequence-checked queue lives: it holds the client's next mutation until its current ticket resolves, replying later with `GenServer.reply/2` once the relay's promotion answer — itself a request message, never a blocking call — has arrived, and pipelined mutations are therefore admitted in arrival order, one at a time. The relay asserts the invariant rather than
...
3. **The monitor's `DOWN` reason alone decides the step.** Exactly the
   ...
   after the request reached it, any crash — is the component's own exit,
   returned as `{:component_exit, reason}` and classified by the component's
   running class: a Store that ended on its own is `store_lost`, and a
...
1. **The relay closes admissions first; then the transport stops accepting —
   ...
   **At the wait's bound the stop proceeds**, and what that costs is stated. A
   ...
   `freeze_deadline`; returned-set cleanup includes every relay request the daemon owner itself sends during that cleanup, such as the no-reply refusal of an unproposed grant, and none of those requests carries an instant of its own; otherwise the owner atomically latches `relay_lost`,
...
| `relay_lost` | 105 | The admission relay exited, or an exact relay control exchange — including a relay step of a serving lease operation past its own five-second instant, or a lease owner's, registry's or connection's reported unanswered relay request while serving — was missing, late, malformed or inconsistent, so admission tickets or retained settlement state cannot be proved complete | `fatal:relay_lost` |
| `connections_lost` | 106 | The connection registry exited, or an exact registry control exchange — including a registry step of a serving lease operation past its own five-second instant — was missing, late, malformed or inconsistent, so connection slots, monitors, buffer control or the required routing mirror cannot be proved complete | — the record would have gone through the interface that is gone |
...
**Two lateness cases select no class.** A lease owner that overruns its own five-second step — a resolution, attachment or retirement acknowledgement — is killed and handled as the session-scoped exit below. A holder connection that overruns its owner-loss close step is killed alone; its exact `DOWN` completes the close, that client observes EOF, and the daemon keeps serving. Neither is `connections_lost`, which names the registry.
...
result, fatal disposition or orderly seal. Under the classification step's own fresh five-second relay-control instant, the daemon sends exact
...
owner_incarnation}`. Only after that acknowledgement does the daemon monitor the returned holder and send it the uncorrelated holder-loss form; a holder that has not acknowledged its close by its own five-second step instant is killed and its exact `DOWN` completes the close, with EOF for that client and no daemon class. A renewal changes
...
classification acknowledgement become prompt/cleanup-only. A holder close already in progress keeps its monitor and is rebound to the same transport-cut instant, at which an unclosed holder is killed and its exact `DOWN` completes the close; no holder-close step can remain at the lease freeze. A classification
...
| --- | --- | --- |
| The dispatcher's release on holder `DOWN`; `Control.invalidate_attachments/3` and `carried_attachment/2` at succession; the per-holder attachment set and monitor state that both owners keep; the transfers released with each attachment; the acknowledged succession-invalidation transaction and holder notification; `Control`'s repetition state; and `Runtime.command_for_daemon/2` with its opaque route token, attachment-invalidated identity, detailed coordinator result and exact pre-call, pre-commit or post-route disposition, plus `Runtime.classify_daemon_result/2` returning `:before_succession_cut` or the exact `{:after_succession_cut, attachment_id, attachment_incarnation}` identity | Core change 1 | The concurrent-attachment cases, the ADR 0028 transfer case, the barrier-serialized scoped-replacement case, the attach connection-loss case, the active-resume succession case without holder `DOWN`, and command cases forcing route-before-cut, cut-before-route, callback-before-nested-fence, real-result classification on both sides of non-prepared succession, prepared succession carrying the attachment, pre-route coordinator death, post-route death, commit-then-crash and exact Control loss while released `Runtime.command/2` stays byte-identical |
...
- **A replayed create returns identity; placement-proved activation repairs.**
  ...
  placement gets `present`, then the exact detailed result `{:error,
  :runtime_placement_mismatch, %{disposition: :no_activation, control_entry:
  :dormant}}`, maps it to correlated `composition_mismatch` with the literal
  bounded message `session cannot be activated by this daemon composition`,
  starts no coordinator, charges no activation and records nothing. A genuinely fresh create asserts
...
| --- | --- | --- | --- |
| **Ordinary session coordinators** | Core's session supervisor, `restart: :temporary`, each with `shutdown: 5_000` | One per active/acquiring entry; at most 64 in daemon use | No death signal reaches the daemon and none is owed. The reference live-resume path detects a dead coordinator with bounded post-attach `session.inspect`; the daemon-only command route independently proves death before a coordinator call as `session_unavailable` and treats any lost reply after a successful route as `admission_unknown`. `quiesce/1` closes the internal start gate, issues every supervisor termination together, then directly kills survivors at the 325,000 ms work cutoff and requires every exact `DOWN` by the 330,000 ms outer instant before fencing |
...
**Inter-component calls.** ### Inter-component exchanges
...
| E5: lease owner → registry (`prepare_resume`, `promote_resume`, `cancel_prepared_resume`) | `send_request` | None; the registry's reply waits on the relay |
| E6: registry → relay (`authorize_resume_ticket`, `wait_for_ticket`, `promote_ticket`, `promote_resume_ticket`, `refuse_resume_ticket`, `settle_ticket`) | `send_request`; one relay step in flight, with a FIFO flow queue | Registry, 5 s while serving; reports `{:registry_relay_unanswered, registry, routing_inc}` to the owner, which selects `relay_lost`; the registry keeps serving |
| E7: connection → registry (`promote_create`, `promote_attach`) | `send_request` | None |
| E8: collaboration owner → registry `close_all` | `{:owner_request, …}` / `{:owner_reply, …}` | Owner, at the close deadline + 500 ms; `connections_lost` |
| E9: connection → relay at request start (`open_ticket`, `bind_ticket_worker`, `open_permit`, `bind_worker`; `socket_connection.ex:787, 798, 1947, 1963`) | `send_request`; ledger phases `:opening`/`:binding`; the worker stays monitored until the bind answer arrives | Connection, 5 s while serving; sends **`{:connection_relay_unanswered, self(), connection_incarnation}` to the collaboration owner** (`state.context.owner`), which selects **`relay_lost`** |
| Unchanged: `register_connection` | Remains a blocking call | Happens before initialize, so no lease is held yet |
| Unchanged: connection → registry output, succession, `lease_route`, facts | Remain blocking calls | The registry is a non-blocking leaf after E6 |
...
concurrent-attachment change with four parts** — supersession stops removing
...
daemon the caller is a relay task rather than the holder, and one daemon-only
command entry plus detailed coordinator result binds that identity to an
opaque route token and distinguishes pre-call coordinator loss, exact
pre-commit succession and post-route uncertainty without changing the embedded
command API — because the first half
...
| Daemon socket output buffer per connection | 4 MiB encoded total; while an attachment is pending or installed or a local succession row remains, ordinary frames may use only `4 MiB - succession_delivery_reserve_bytes`. The derived reserve is the maximal encoded generation-2 succession `detached` frame plus one maximal correlated succession-reply slot reused serially by at most 32 predecessor lease-authorized mutation origins; post-cut request admission pauses until the row clears, and the reserve is not a separate advertised limit | ADR 0032 |
| Resident window per session | 4,096 events and 16 MiB encoded | ADR 0032 |
| Aggregate encoded-byte commitment per daemon | 512 MiB across charged output/window bytes, pending admission bytes and unused per-attachment succession reserves | ADR 0032, its queue-ownership and attachment-lifecycle sections |
