...
M5 accepts two decisions before dependent work:

| Decision | Owner and acceptance point | Effect |
| --- | --- | --- |
| [**ADR 0032**](../adr/0032-daemon-attachment-residency-and-replay.md#concept) | Maintainer, before M5 acceptance; must bind every residency number below and the generation-2-only negotiation rule | The Unix-domain-socket transport carrying ADR 0023 unchanged, generation 2's daemon methods, refusal of a generation-1-only client, owner-only peer access, the bounded socket path, race-free attach with a resident window, bounded queues, detachment at the last emitted cursor, idle eviction and `cursor_expired` |
| [**ADR 0033**](../adr/0033-collaboration-controller-lease-and-takeover.md#concept) | Maintainer, before M5 acceptance; must fix the lease terms below | One daemon-owned controller lease per session held in daemon memory with an incarnation-scoped writer epoch, admission that binds the holder connection, held state, unexpired term and writer epoch, explicit takeover with an epoch advance, cross-process abort through the core's own cancellation, and no authority from content, metadata or order |

ADR 0031 is not an M5 prerequisite. The daemon-grade store, its contract
experiments and its migration belong to the successor milestone, and that ADR
stays Proposed until then; M5 runs the daemon on the local adapter.
...
| `loopex_store_local` | The unchanged local adapter and its writer marker, which the daemon holds for the root's lifetime | Any daemon fact, lease or residency state |
| `loopex_daemon` | Process and socket lifetime, peer-credential check, generation-2 negotiation, attachment residency and eviction, the in-memory controller lease and writer-epoch check, session list and stop, and diagnostics | Store or coordinator internals, a second loop, policy selection, host identity or any durable record |
...
minimal application → narrow core multi-attachment support → daemon lifetime
and socket over the local adapter → collaboration lease and takeover →
residency, replay and backpressure → two-process workflow with the reference
CLI and TypeScript consumer → integrated audit → independent review. Prove
lifetime, refusal of a second daemon, reopening under the foreground server
after an orderly stop, independent simultaneous core attachments,
...
lease record lives in the daemon's memory, keyed by session, for the daemon's
lifetime; one owner process per session serializes its lease transitions with
its admission handoff. For every existing-session mutation, the daemon checks
the requesting connection identity, current epoch, held state and unexpired
term together before forwarding to core; an epoch alone is not authority, and
an epoch minted by a previous daemon incarnation never matches. Attach reuses
the runtime's cursor transaction; the core keeps attachments independent,
while the daemon buffers and evicts above it and never reads coordinator
state. Cross-process abort forwards the existing durable `session.abort`
command; no daemon-side cancellation path exists.
...
| --- | --- |
| 1 | Real daemon process per state root; sessions progress with zero attachments; orderly stop releases the writer marker and records nothing false; abrupt kill followed by restart recovers every session under the same placement identity with no duplicate effect; a second daemon on the same root refused by the held marker; after an orderly stop the foreground server and the reference CLI reopen the same root and resume a daemon-created session under the same placement identity with identical replay; session list, open and stop through the socket only |
...
| 3 | One lease per session in daemon memory with observers attached; connection identity, epoch, held state and unexpired term checked together before core admission or durable write, including a known current epoch sent by an observer; takeover only after release or expiry with an epoch advance before the successor's first command; killed controller fenced and its late commands refused; daemon restart leaving every session uncontrolled with an epoch from the previous incarnation refused; controller abort cancelling work dispatched under an earlier process with a truthful cleanup outcome; no control from content, metadata, answers or attachment order |
...
daemon, socket, controller and observer rules, residency and restart
behavior, the source-only `v0.2.0` release sequence, current M5 information,
relevance, working links and directory-index routing where applicable, and
stale foreground-only claims.
...
generation 1's method set with the exact-generation rule; a generation-1
...
generation 1 unchanged. The private journal, its adapter and its format are
unchanged; the embedded API, public events, snapshots, artifact formats and
executor protocol are unchanged. Source VERSION is distinct from protocol
generation, provider build and schema digest.
...
then resume a daemon-created session with identical replay. Rollback is
stopping the daemon: removing it restores the M4 foreground server and CLI on
the same root with no durable dependency on residency or lease state. Restore
floor, version and inventory protections through governed transactions. No
in-place downgrade, installed-data migration or service-manager claim.
...
and eviction policy, three CLI commands and the consumer's socket mode
justify growth. No lease, transport or residency policy in core; no second
loop, event dispatcher, cancellation path or protocol codec; no transport
registry, plugin socket layer, generic service framework, store adapter,
control store or durable daemon record; no daemon-side session state beyond
the lease and residency facts it holds in memory. Implement lifetime,
transport and collaboration once against the local adapter. Raw line count is
a review signal; behaviour and measured limits govern.
