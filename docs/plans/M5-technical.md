...
takeover prompt retains its method, preallocated command ID and
...
command input. Core command replay returns the same accepted or refused
admission and cannot duplicate the mutation. The client advances `next_step`
only after that decisive admission or the exact durable prerequisite that makes
the following step valid. It never retains a writer epoch as reusable
authority.
...
| A controller whose current planned step is `sent_unconfirmed` | Reconnect, reacquire and attach at the retained cursor, using dormant recovery only for `run` or `resume`, then re-present the exact method, durable command ID and retained canonical semantic input under a fresh request ID and the fresh writer epoch. The idempotent admission replay decides accepted or refused even when that command emits no public event. Acceptance advances to the retained next step once its exact prerequisite holds; refusal ends the plan; failure to obtain the admission reply by the fixed deadline exits non-zero with that step unresolved. |
...
| `attach --take-over` after its optional prompt reached `sent_unconfirmed` | Reacquire and attach without resuming, then re-present the exact prompt command and semantic input with a fresh request ID and writer epoch. Its replayed admission decides without a duplicate. A dormant result takes the same bounded release-and-exit path. |
