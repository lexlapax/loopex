...
- **the extracted-build witness separately compares a canonical manifest**,
  ...
  `mode` is the lowercase octal permission mode as minimal base-8 digits with
  no leading zero except the value `0`, `path` is relative to the
...
both marked status files byte-for-byte from the tested files plus the exact
marked-block bytes in the administrative files, and requires the root README to be the sole path outside
