...
and clear its retained repetition and monitor state. Pending attach callers
receive `attachment_superseded`; an installed embedded handle becomes stale.
...
and closes that connection. This is the fourth generation-2 `detached`
occasion. Unlike the three single-attachment eviction occasions, a succession
may notify every connection attached to that session; no unrelated session is
affected.
...
| Core succession invalidates an installed attachment | A received `detached` record or bare EOF is attachment loss, never evidence that the session is dormant. Reconnect under the same streaming deadline at the retained last-emitted cursor. An observer attaches without activating; a controller reacquires a fresh epoch, attaches and inspects before continuing. Neither role sends `session.resume` solely because succession invalidated the old handle. |
...
`SIGTERM`; a directly executed escript handles only `SIGTERM`. Direct
`SIGHUP` is abrupt death and direct `SIGQUIT` is ignored after handler
installation. Direct `SIGINT` remains the emulator's reserved break behavior and
...
`stderr`, and a raised error, throw or exit is caught into the same step
family together with the exact partial pid inventory already acquired. In
particular, `File.mkdir_p!/1` raising `File.Error` during placement selects
`placement_lock_failed`, rather than escaping as `owner_lost`.
...
| `SIGQUIT` | Orderly stop: forwarded as `SIGTERM` | Ignored after daemon handler installation because the installation removes OTP's default handler; before removal, stock OTP handles it as status-`0` `erlang:halt/0` with no dump |
...
`SIGQUIT` asserts the process stays live and emits no stop record after
handler installation. The direct-`SIGINT` case asserts only that the
...
handler installation; none claims interrupted-import cleanup.
...
- **Signals, one case per signal per route and phase, and the route is what
  ...
  process stays live with no stop record after handler installation. Those
  outcomes make "the daemon installs on `SIGTERM` and nothing else" a checked
  claim rather than a preference. The direct-`SIGINT` case asserts only
...
- **Offline import interruption is nonzero and never partial.** Six forced cuts
