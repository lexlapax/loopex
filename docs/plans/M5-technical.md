...
| Socket path | `--socket` | `<root>/daemon/daemon.sock` | `socket_path_too_long`, `socket_permission_unverified`, `invalid_socket_path` when an explicit path's bytes are not valid UTF-8 or it is outside the root's `daemon/` directory |
...
- **during offline import**, which has no readiness or wire surface:
  ...
  reader — and `session_index_write_failed`;
...
| `loopex daemon prepare-index`, its state-root-only grammar and pre-effect UTF-8 validation, exclusion order, strict legacy reader and whole-import refusal, success and residual outcomes | `docs/operator/daemon.md`, migration section |
