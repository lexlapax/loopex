...
| --- | --- | --- |
| `loopex` | Durable session truth, the race-free attach barrier and cursor, independent concurrent attachments to one session **and their release — the dispatcher monitors the attaching process and drops its attachment on `DOWN`, which is what replaces the supersession this change removes, with `Control`'s own attachment and repetition state held per holder rather than per session — the holder pid owning the entry and releasing it on `DOWN`, the handle's `attachment_id` resolving it at the command path, which discards its caller because under the daemon that caller is a relay task and not the holder**, the read-only session-existence query, **`Loopex.Trace.exclude_self/2`, which installs both the match-specification exclusion ADR 0030 names and the process-level exclusion its callees need, before any message is delivered, and the `Loopex.Trace.Entry` clause that redacts a keyword list's values under their own keys**, **the bounded `quiesce/1` that settles every active coordinator or names what it could not — executing in its caller's process, not in `Control`, which is on every coordinator's commit path — together with the one coordinator change it needs in two halves, a drained abort pausing where cleanup would have begun and its commit presented once rather than retried on an ambiguous answer, and the one new `Control` read it enumerates from — a transient, core-internal drain context carrying the Store handle `Control` holds beside each entry's status, coordinator pid and owner, because `%Loopex.Runtime{}` carries a supervisor and a token and no store**, **`create_session_detailed/3` and `resume_session_detailed/3`, the two runtime-side functions carrying `disposition` and `control_entry` beside the unchanged create and resume**, the per-attachment event-count dispatcher queues, cancellation and recovery | A lease, a transport, a byte limit, residency policy or any daemon fact |
...
- **`docs/operator/daemon.md` must state the maximum graceful stop, and the
  figure it states is the one the implementation *measures*** — not one this
  plan derives. The maintainer withdrew the derived sum on 2026-09-20 for the
  reason the lifecycle section gives, so this row carries the obligation
  instead of the number: at closure the page names the measured maximum, the
  conditions it was measured under, and the two bounds this pair does fix
  beside it — the **fail-stop** exit at **5 s + 30 s = 35 s** and the **Store
  stop's** fixed **30 s**. It must also say that the measured figure is
  **not** read off the daemon's own
...
six changes, six rows**, matching the surfaces table below one for one. An
earlier revision had five rows for six changes, with one of the five being
half of change 4 and two changes carrying no core row at all.
...
| --- | --- | --- | --- |
| Concurrent attachment (core change 1) | `apps/loopex/test/concurrent_attachments_test.exs` (**new**) | `two attachments to one session coexist without replacement`; `one detaching leaves the other delivering`; `an attachment is released when the process that attached it exits`; `an attachment released by its process exiting leaves no open transfer`; `the installed attachment records the attacher pid and the monitor reference`; `a controller and an observer attached to one session both remain usable`; `Control validates each holder's command against that holder's own attachment`; `a command sent from a process that is not the holder resolves by attachment_id and is admitted` — the case a pid-keyed `Control` fails, and the one that forced the dual key; `replace true supersedes only the attaching process's own prior attachment and releases its transfers`; `replace true from a second process supersedes nothing of the first's`; `replace true leaves every other connection's attachment to that session delivering`; `the dispatcher holds no attachment for a dead attacher`; `one backpressuring does not stall the other`; `each carries its own cursor and incarnation` | fast |
| Read-only existence query (core change 2) | `apps/loopex/test/session_existence_query_test.exs` | one per result of the closed set | fast |
| **`quiesce/1` and the coordinator change it needs** (core change 4) | **`apps/loopex/test/runtime_quiesce_test.exs`** (new) for the drain, and **`apps/loopex/test/cancellation_test.exs`** (existing; the file that already drives an abort against a receipt arriving mid-reduction) for the coordinator half | `admits every abort before any cleanup begins`; `a coordinator commit during the abort admission is served by Control and its session settles`; `a drain task that crashes fences its session and does not reach the caller as an exit`; `a drained abort commit is presented once and never re-presented on commit_unknown`; `a fresh create executes nine store calls, ten with the retry`; `attach executes one store call over an empty session and two over eight events`; `resume of a twelve-record session executes eleven store calls`; `the reserve-completion callback commits three times on a refused fact and twice on a committed one`; `an abort admission whose reply path settles an open model attempt commits twice and stops`; `quiesce admits no abort into an acquiring entry, and terminates and fences it instead`; `a fence executes three store calls at worst`; `releases cancellation concurrently once every admission resolves`; `an empty active set drains with a zero budget`; `reports a Control entry whose coordinator has died as absent`; `reports an unavailable Control entry as absent and fences it`; `reports an acquiring entry as unsettled and fences it`; `an acquiring entry whose owner_ready cast is in flight is terminated and reported unsettled`; `settled is read from the coordinator's status, not from the cancellation finishing`; `a cancellation that finished with its terminal commit held is unsettled, not settled`; `fences an unsettled session and refuses its paused transaction as stale`; `fences an absent session too`; `fences a settled session too, and refuses its straggler commit released during the store phase`; `reports not_needed only where the fence enumeration holds no entry`; `fences every unavailable entry, including one whose coordinator was terminated`; `reports an awaiting_owner_barrier entry as unsettled and fences it`; `terminates and fences an entry that appeared during the admission and the cancellation`; `a fence refused stale_owner_epoch is superseded with no second attempt`; `a fence refused stale_journal_version is superseded too, not an error`; `a fence answering commit_unknown is resolved through transaction_status under the same derived tx_id and reported unsettled with fences[id] == {:unknown, head}`; `a fence id recomputed from the head alone matches the one the drain used`; `a session whose abort admission is ambiguous has no cleanup released and is fenced`; `an admission task shut down mid-call still leaves the coordinator admitting the abort and pausing at the split`; `the status census is bounded by one deadline, not one per session` — five hundred and twelve enumerated sessions whose coordinators never answer, asserted to be reported `unsettled` within one census deadline rather than within five hundred and twelve of them; `the enumeration carries the store handle the fence commits through`, asserted by a drain that fences with no store handle of its own; and, in `cancellation_test.exs`, the coordinator half: `a drained abort commits without beginning cleanup`; `a client abort still begins cleanup on its commit reply path` — the pair that proves the split changed the drain and nothing else | fast |
| **`create_session_detailed/3` and `resume_session_detailed/3`** (core change 5) | **`apps/loopex/test/session_detailed_results_test.exs`** (new) | `a fresh create answers disposition created`; `a replayed create with the original command_id answers disposition replayed and starts no coordinator`; `a resume of a dormant session answers control_entry dormant`; `a resume of an already-active session answers control_entry active`; `create_session/3's own return is byte-identical to the released shape` — the case that makes the addition provably additive rather than a facade change; `resume_session/3's own return is byte-identical too` | fast |
| **`Loopex.list_sessions/2`** (core change 6) | **`apps/loopex/test/session_listing_test.exs`** (new) | `a bounded listing stops after the bound-plus-first valid entry`; `it reports that it stopped early`; `a root within the bound reports that it did not`; `the arity-one form is unchanged over the same root` — the pair that proves the addition is additive; `a bounded listing over a root past the bound performs exactly bound-plus-one entry reads`, asserted by tracing `Loopex.SessionDirectory` and counting `read_entry` calls over a root seeded with far more, never by timing it | fast |
| **`Loopex.Trace.exclude_self/2`, `Control`'s excluded-pid set and `Entry`'s keyword-key redaction** (core change 3) | **`apps/loopex/test/trace_session_test.exs`** (existing; extended) | `the named MFAs produce no raw message under an explicitly named module, before any process flag is set`; `an excluded process produces no trace message`; `the exclusion survives a tracer restart`; `a new session skips an already-excluded pid`; `fails closed while the tracer is absent`; `fails closed while Control is unavailable` — the case that constructs a `Control` restart, and which therefore asserts what that costs: every child after `Control` restarts with it, so every session coordinator in that runtime is gone and the case starts a fresh session rather than reusing one;  `the excluded set returns to baseline after the sender exits`; `a keyword list's value is redacted under its own key`; `the same value under a key naming nothing is rendered, so the case cannot pass vacuously` | fast |
...
resume's eleven over a twelve-record session; a fence's three at worst.
...
**The embedded API's *shape* is unchanged and one of its *behaviours* is
not**, which an earlier revision folded into "the embedded API is unchanged"
and thereby denied. `Loopex.attach/3` (`loopex.ex:134`) keeps its arity, its
arguments and its result. What changes is what a second attach to the same
session does. Today the dispatcher supersedes unconditionally: it splits
`state.attachments` on `session_id` alone
(`runtime/event_dispatcher.ex:812-818`), and no `replace` option exists
anywhere in core — `Loopex.attach/3`, `Runtime.attach/3` and the dispatcher
mention none. So an embedded caller that attaches twice for one session, from
one process or from two, sees the first attachment superseded and its
transfers released. **After core change 1 it does not**: supersession
requires `replace: true` and is scoped to the attaching process's own prior
attachment, and a second attach without it leaves both attachments live. That
is the behaviour the daemon exists to have, and it is the one the migration
note below names.
...
caller that relied on `Loopex.attach/3` superseding its own earlier
attachment to a session — the only way core offers today, since the
dispatcher supersedes on `session_id` alone with no option to say otherwise —
must **pass `replace: true`** after core change 1, and must pass it from the
**same process** that holds the attachment it means to replace. A caller that
does neither gets two live attachments where it used to get one, which is a
resource leak rather than a refusal and so will not announce itself: the
older attachment keeps its resident window and its queue until its process
ends, and the dispatcher's `DOWN` release is what eventually collects it.
Callers that attach once per session, which is every caller in this
repository, need no change at all. **Rollback is symmetric and needs no
note**: a root carries no attachment state, attachments being runtime-local
and transient, so reverting core change 1 restores the old behaviour for
whatever is running next.
...
3. **Start the credential routing registry, the custody process and the
   ...
   left to the process table: it is one of the eleven links, ADR 0034 now makes
...
6. **Start the admission relay, then the connection registry.** They are the
   ninth and tenth of the eleven linked processes and the last two before the
   listener. The relay needs the runtime step 4 returned, and it must exist
   before any connection can be accepted, because every call that changes core
   state goes through it. The connection registry must exist before the
   listener accepts, because the slot it reserves is reserved **at** `accept`
   and the monitor it takes is taken there too — a listener accepting before
   it exists would accept connections nothing owns. An earlier
...
or not that session is active — which cannot happen before a client connects.
...
- **Startup does not run in `init/1`, because a monitor taken after
  ...
  **Step 5's wait is what makes the monitor provably installed**, and it is
  ...
  wait is bounded like every other wait in this plan — an absolute deadline
  the implementation measures — and a
...
- **Tickets cover every core call that mutates `Control` or the journal, plus
  ...
  **The tenth is `session.attach`, and it is ticketed for the cut alone — the
ticket is written by the connection process and the call is made from that
same process, never from a relay task.** The dispatcher monitors **its
caller** (`event_dispatcher.ex:186-192`), so an attach made from a short-lived
relay task would be released the instant that task exited, which is
immediately. Caller-is-holder is therefore kept exactly as the attachment
rules above state it: the connection process calls, the dispatcher monitors
it, and its `DOWN` is the release. What the relay holds for an attach is the
**ticket alone**, so the cut can account for a call in flight; it neither
performs the call nor owns its lifetime. An earlier revision proposed an
...
- **The ticket is recorded and the call is started before the
  acknowledgement, both inside the one callback.** The lease owner — or, for a
  create, the connection process — **calls** the relay and waits, and the
  relay writes the ticket row **and spawns the monitored task that performs
  the core call** before it replies. A cast would have left the window open at
  ...
  acknowledgement therefore means two things at once — the ticket exists and
  the call is running — which is what every later reading of the ticket set
  assumes.
...
- **At most one unresolved mutation ticket per session**, which is the
  ...
  it. The **lease owner** is the only process that makes ticket calls for a
  session, so it is where the queue lives: it holds the client's next mutation
  ...
  **Two of the ten are not mutation tickets and the rule does not reach
  ...
  what bounds creates is the activation ceiling and its reservation.
  `session.attach` is ticketed for the cut alone and is not performed by a
  relay task at all; what bounds attaches is ADR 0032's attachment ceiling and
  its reservation. Neither is a mutation of an existing session's durable
...
- **The admission cut is the relay's, because the relay is the only process
  ...
  What the cut buys is therefore narrower than that sentence claimed, and it
  is the true thing: after the cut **no new work enters core through the
  daemon**,
  so the set quiesce is about to read can only shrink from admitted work
  finishing, never grow from new work arriving — except by a ticket still
  settling, which is why the termination and the fence re-read the
  enumeration and why the
  create residual below is stated. That is why every call that mutates
  `Control` or the journal goes through this one process. The narrower claim
  is the true one; the transfer methods above are the reason it has to be
  narrow.
...
as **one collective sweep, not one at a time**, against the teardown deadline:
...
currently stopping** — one pid at every step but the teardown's lease-owner sweep,
...
2. It `spawn_monitor`s a **helper** whose whole body is
   ...
   what is left of that step's own deadline and no component carries a budget
...
this step is stopping — on the set of them, in the teardown's lease-owner sweep, by
...
step, and states no per-step seconds and no total. What replaces the published
sum is a closure obligation: the operator page carries the **measured**
maximum graceful stop, taken from the implementation, and the implementation
states every bound it uses as an **executed count** under the rule the
verification section keeps. Two numeric bounds survive here because neither is
derived from a drain: the **fail-stop** exit at **5 s + 30 s = 35 s**, and the
**Store phase's fixed 30 s**, which is the adapter's own `@call_timeout`
(`apps/loopex_store_local/lib/loopex/store/local.ex:65`) and the one stop whose
`terminate/2` releases a durable exclusion.

**What the cut guarantees.** The owner calls the relay, which closes
admissions and acknowledges that **state change** — one synchronous
acknowledgement, not a wait for work. After the acknowledgement **every
request on an open connection is refused `daemon_stopping`, whatever its
method**: the ten state-changing calls, the queries, and ADR 0023's three
artifact-transfer methods alike. That refusal is a **synchronous
admission-state check the connection layer performs before it dispatches
anything**, not a property of the relay's ticket set — an earlier revision
gated only the ticketed methods and left reads and transfers entering core
after a cut that claimed to have closed it.

The ten state-changing calls carry tickets, and the tickets already inside
core when the cut answered are **awaited for a bound the implementation
measures**, after which **the stop proceeds**. It does not wait for them to
settle: an earlier revision said the cut "answers only when every ticket it
holds has settled", which a resume paging a long history can defeat, and which
contradicted every witness written against the bounded wait. What proceeding
costs is stated where the classification is: a ticket still running leaves its
session `:active` or `:acquiring`, and both are classified and fenced.

**What the abort admission guarantees.** Quiesce admits the existing abort
command — a fresh core-minted command ID, and a commit that is **presented
once** rather than re-presented on an ambiguous answer, the two halves of core
...
(`control.ex:575-591`). A session that is not `:active` has no ready
coordinator and no run to abort.

**The invariant is global, and it is the one the Concept promises.**
Cancellation is released **only when every admission is durably decided**. If
any admission is ambiguous or unanswered, **no cleanup is released for any
session**: the census is terminated and fenced instead. An earlier revision
...
**Then termination, then the fence, over a fresh enumeration.** Every
enumerated coordinator is terminated and every `DOWN` awaited **before** any
fence commits, so no fence races a live writer; the enumeration is taken again
at that point rather than reused from the admission step, because `Control`
keeps serving while the drain runs and an entry can appear during it. Both are
bounded by **one absolute deadline the implementation measures**, and
...
**What the fence guarantees.** One `advance_owner` per enumerated session,
its identity derived from `(session_id, expected_owner_epoch,
expected_journal_version)` so a successor can recompute it, and after it no
transaction for that session can commit — the epoch has moved and the adapter
refuses it `:stale_owner_epoch` (`state.ex:548-551`). A **committed** fence
...
resolved by one `transaction_status` query under the same derived identity,
and if still unknown is reported `{:unknown, head}` with the head it read.

**One residual is stated rather than closed**, and it covers both
activation-capable calls: `session.create` and `session.resume` each make
Store calls before the entry that makes a session enumerable is written
(`control.ex:959-965` and `:896-897` before `await_owner/9` at `:1176-1195`;
`:310` before `:1030` and `:1189`). A call inside them is invisible to any
enumeration, so it is neither terminated nor fenced. What holds for it is
crash-equivalence: the runtime kill ends a non-trapping coordinator at once,
at most one already-delivered transaction linearizes, and it lands as an
ordinary durable fact — a genesis, an owner advance or a command admission —
with **no terminal claimed**, read as history by the next activation
(`session_coordinator.ex:1186-1203`). For a create whose client never learned
the session ID, ADR 0032's recovery by `command_id` applies in addition. A
late `session.attach` is not in this class: `begin_attach/3` requires the
entry to be `:active` already (`control.ex:1237`).
...
**one deadline, `teardown_ms`, covering every non-Store action**, sized for
work that is milliseconds by construction — the notification writes are one
...
closes the descriptors it holds. The figure the implementation settles on is
...
**The teardown deadline starts when `quiesce/1` returns**, and it covers every
...
1. **The listener stops accepting, and the relay closes admissions — a
   ...
   into one step it could not possibly contain. The owner calls
   ...
   answers at once**: an acknowledgement of a state change, bounded on the
   relay's responsiveness and on nothing else. The owner then **waits for the
   tickets already inside core to settle**, for a bound the implementation
   measures.
...
4. **The connection registry stops, then every lease owner as one sweep, then
   ...
   connection it monitored is already gone. Every lease vanishes with the
...
6. **The remaining composed edges stop, then the Store last.** Reverse start
   ...
   **What refuses its commit is the fence, not a dead Store**, and an earlier
   ...
   acceptable transaction. It is refused because **the fence has already moved
   ...
   after the Store stop had finished.
...
returns it as `budget_ms`; the daemon's own `teardown_ms` starts when the
call comes back. A long derived budget is not a hazard the argument would have
...
race first, `:not_needed` where the fence's own fresh enumeration holds no
entry for the session at all, and
...
1. **The abort admission: every live coordinator admits
...
2. **The cancellation: it is released, concurrently,
   only once every admission has resolved.** Core waits for the admission to finish for every
...
3. **Terminate every coordinator, then fence every session — two steps, in
   ...
   **every enumerated coordinator** — settled, unsettled and acquiring alike —
   and waits for every `DOWN`; that is the **termination**, bounded by the deadline the implementation
   measures and issued concurrently across them — a coordinator does not trap
   exits, so each dies at once, though `DynamicSupervisor.terminate_child/2`
   serialises them through the one session supervisor. Only then does it fence
   **every session it enumerated**.
   ...
   **Termination and the fence run over a *fresh* enumeration, not the one the
   admission used.** `Control` keeps serving while the drain runs — which is the whole
   point of quiesce being outside it — so entries can appear during the admission and the cancellation, and a create admitted just before the cut is exactly what appears.
   Termination therefore takes the enumeration again at its own start, and it
   and the fence both work from that list. An entry that arrived during the
   cancellation is terminated and fenced like any other; the `:not_needed`
   no-entry row is read from **this** list rather than the earlier one.

   **The fence covers every session in that enumeration — settled, unsettled and
   ...
   enumerated session, issued concurrently — and after `quiesce/1`
   ...
   `:not_needed` therefore survives for one case only: a session the
   **fence's own fresh enumeration holds no entry for**. Not an `:unavailable` entry,
   which an earlier revision used and which proves nothing about whether a
   coordinator once existed; no entry at all.

   **One residual survives the fresh enumeration, it covers *both*
   activation-capable calls, and it is stated rather than closed.** An earlier
   revision wrote it for `session.create` alone and leaned the whole
   disposition on replay by `command_id`, which is a create's recovery and not
   a resume's. Both calls have the same window, for the same reason: the entry
   that makes a session enumerable is written late.

   - **`session.create`** makes up to **three** Store calls — the
     `runtime_command` at `control.ex:959-965` and the genesis commit at
     `:896-897`, re-presented once — before `start_owner/4` reaches
     `await_owner/9` (`:1176-1195`), where the `state.sessions` entry is
     written.
   - **`session.resume`** makes its `Store.runtime_command` at `control.ex:310`
     before `start_resume_owner` reaches the same `do_start_owner` path, whose
     `:acquiring` entry is written at `:1030` and `:1189`.

   A call inside those Store calls is invisible to any enumeration, so it is
   neither terminated nor fenced, and its coordinator
   may start after that read.

   **What actually holds for it is crash-equivalence, not replay**, and that
   is the disposition stated: the runtime kill in the teardown ends such a
   coordinator **at once**, it being non-trapping; at most **one** transaction
   it had already delivered can linearize afterwards, the owner lane being
   serial; and that transaction lands as an ordinary durable fact — a genesis,
   an owner advance or a command admission — with **no terminal claimed**. The
   next activation reads it as history, `discover_and_advance_owner/1` reading
   the head it finds (`session_coordinator.ex:1186-1203`). Nothing false is
   recorded and nothing is lost.

   For a **create whose client never learned the session ID**, ADR 0032's
   *recovery procedure for the crash cut* applies **in addition**: the client
   replays its `session.create` under the same `command_id`, and the daemon
   repairs the directory entry and index row. That is a second recovery for
   one of the two calls, not the reason either is safe.

   Bounded, in both cases, by the call's own remaining depth — the rest of the
   rest of the create's own sequence, and a resume's.

   **A late `session.attach` is not in this class**, and the reason is worth
   one line: `begin_attach/3` requires the entry to be `:active` already
   (`control.ex:1237`), so an attach cannot bring a session into existence
   behind the enumeration. It either finds an entry the enumeration also found
   or is refused.
   ...
   **The classification quiesce returns has three lists, not two, and it is
   ...
   `:833-857`), and a session may have no entry at all, so the drain says what
   it does with each:
   ...
   | `:awaiting_owner_barrier` | A coordinator has died with its **owner group still alive** and a caller still waiting (`control.ex:835`, `:1040`, `:1046-1053`, consumed at `:833-857`) — reachable exactly when the bounded wait proceeds with a resume in flight | `unsettled`; no abort is admitted, there being no ready coordinator to admit one | **Yes, and the fence is what makes it safe.** Its coordinator is already gone, so termination has nothing to reach; what could still write is **the dead coordinator's at-most-one already-delivered transaction** — the same case every other fenced row rests on — and nothing else, because no owner group or worker can commit: `Store.transact` has one caller in core today (`store/owner_lane.ex:121`), and `OwnerLane.transact` is called only from the coordinator (`session_coordinator.ex:1242`, `:1363`, `:1761-1762`) and `Control` (`control.ex:1680-1681`). Core change 4 adds the second caller — the drain's own fence tasks, calling `Store.transact` directly on the handle the enumeration hands them — and it is named here rather than left to contradict this count, because it weakens nothing: the fence is the writer this row protects the session *with*, it runs after the coordinator is gone, and it is the only thing core change 4 lets reach the Store outside a lane. The owner group that outlived the coordinator unwinds on its own clock past the runtime kill, which ends the root only, but it writes nothing. The fence's moved epoch is what refuses that one transaction `:stale_owner_epoch`. An earlier revision credited the kill, and the one before it named the owner group as a writer |
   | No entry | The session is dormant in this daemon, or belongs to no daemon at all | In no list; quiesce enumerates `Control` and nothing else | No — the **only** `:not_needed` case, and it is read from the fresh enumeration termination takes, never from the admission's snapshot |
   ...
   | **A fence that did not answer** inside its bound, its task shut down **after** the head was read | The fence's outcome cannot be read | `unsettled`, `{:unknown, head}` | The attempt was made; the domain stays fenced |
   ...
   **The census as a whole is bounded by one absolute deadline, not by 5,000
   ms per session.** The per-coordinator bound is the coordinator's own and is
   not the drain's; taken one session after another it would make the census
   cost the number of enumerated sessions times that bound — up to 512 of
   them — which is the arithmetic this plan stopped doing. So the reads are
   **issued together** and waited for against **one absolute instant the
   implementation measures**, computed once when the census begins, and a
   session whose status has not answered by it is `unsettled` under the same
   fail-closed rule that covers a refusal or a timeout. Nothing is lost by
   that: a status that did not answer was never going to be read as
   settled.
...
What it asks `Control` for is **one** bounded read, made **twice**: the
enumeration — every session entry with its status, its coordinator pid and
its **owner**, the owner being what the per-session calls need to identify
themselves as current. Once at the start of the drain, and again at the start
at termination, for the reason given there. Everything else the drain does —
admitting, cancelling, terminating, fencing and reading status — is
per-session calls the caller's own tasks make **directly to coordinators and
to the Store**, never through `Control`. (An earlier revision said "exactly
two bounded reads" and then listed one and a negation.)

**The enumeration carries the drain context, because the runtime reference
does not.** `%Loopex.Runtime{}` is `defstruct [:supervisor, :token]`
(`runtime.ex:46`) — a supervisor pid and a token, and nothing else. The Store
handle lives in `Control`'s own state (`control.ex:218`), and the drain needs
it directly: the fence is a Store transaction and the `{:unknown, head}`
resolution is a Store query, and both are made by the caller's tasks rather
than through `Control`. So the enumeration answers a **drain context**, not
only a session list: the Store handle `Control` holds, and per session the
status, the coordinator pid and the owner. It is core-internal by
construction — one part of core answering another, in the caller's process,
crossing no durable, public or executor boundary — and it is **transient**,
valid for the call that received it and never retained by the daemon, which
holds no Store handle before the drain and holds none after it. That is part
of core change 4 with the enumeration itself, and the ownership table names it
there.
...
in the ownership table beside `quiesce/1` itself, because a change nobody
listed is a change nobody reviews.
...
daemon without artifact transfers. Earlier revisions said six, then eight,
then eleven here, each being the count from a design with fewer linked
processes; the last of them had already added the connection registry without
adding its class to this list. In every one of
...
3. **Stop the executor** — on every class but `executor_lost`, where it is
   ...
   its own link until this path's own deadline and killing on expiry. That
   deadline is **5 s**, named here rather than measured, because on this path
   it covers one stop and nothing else; it is started when this path began. This is not tidiness: the executor is
...
6. Write the class on `stderr`, then **halt with the non-zero status** for
   ...
   **A fail-stop has an operator bound too, and it is much smaller than an
   ...
   this path's own 5 s, and the Store stop, in its own fixed
...
yet, since none is started before a client's first lease operation and no
client has connected — and **every pid the
...
**releases the marker on the ordinary path**, which is what lets an operator
fix the cause and try again without a recovery step.

**With one residual, and it is the orderly stop's residual reached by the
other road.** The Store stop here is the same fixed 30 s, so a Store that does
not release within it is killed and the marker it held survives — exactly the
stale marker ADR 0031's verified recovery answers, reclaimed where the
recorded holder is proved dead and refused `store_writer_unverifiable` where
it cannot be decided. "Leaves no marker held" is therefore a claim about the
path, not about every outcome, and the witnesses below assert the path. No
second rule is needed and none is stated: a failed start that wedges the
filesystem leaves the root in the state a wedged orderly stop leaves it in.
...
| --- | --- | --- |
| The dispatcher's release on `DOWN`, the attacher pid and monitor reference it keeps on the attachment to do it, the transfers it releases with it, **and `Control`'s per-holder attachment and repetition state** | Core change 1 | The concurrent-attachment cases, the ADR 0028 transfer case, the scoped-replacement case, the attach connection-loss case, and the non-holder command case that resolves by `attachment_id` |
...
- **One class per linked component of the fixed set.** Each is
  ...
  opening that root with nothing to recover; ten of them in a daemon with
  ...
  classes — with the two that reach **no** client asserted to send nothing and
  still exit with their class, since the listener and the connection registry
  are what a record would have gone out through. The set is closed for the
...
| **Connection registry** | Daemon owner, tenth, beside the relay and before the listener | Daemon owner | Orderly step 4, first — after step 3's records and closes, which go through the buffer-control interface it owns | `connections_lost`, daemon-fatal: it owns every connection's monitor and the buffer-control interface, so a daemon without it can neither release a slot nor stop writing to a gone peer. **No client can be told this class**, for the same reason as the listener's: the write goes through the interface that is gone |
| **Listener** | Daemon owner, last of the fixed set | Daemon owner | Step 1 stops it accepting; step 3 closes it, leaving the pathname | `listener_lost`, one of the two fatal classes no client can be told |
...
which is a race. **One bounded `quiesce/1`, the one coordinator change it needs and the one
`Control` read it enumerates from** — the
...
reachable from outside a drain, and a plain-data projection of `Control`'s
session entries, which is the smallest thing that can answer "what is there
