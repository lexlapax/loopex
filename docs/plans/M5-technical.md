...
- **`docs/operator/daemon.md` must state the maximum graceful stop as the sum
  ...
  **`budget_ms` + 555 s**, being 5 s for closing admissions, 300 s for
  settling what was already admitted, 120 s for abort
...
| 2 | **Abort admission** | **120 s** — **four** Store calls deep, at 30 s each; **per phase, not per session**, because the sessions are admitted **concurrently** | The admission waits behind the **callback** the coordinator is already running, which is up to three calls, and then makes its own, which is one |
...
> **Maximum graceful stop = 5 s + 300 s + 120 s + `budget_ms` + 90 s + 5 s +
> 5 s + 30 s = `budget_ms` + 555 s**, and `budget_ms` is the one term an
...
- **Phase 2 is four, and the unit that blocks is the callback rather than the
  ...
  to completion. A callback can make more than one adapter call. The deepest
  one traced makes a **commit** — `resolve_transaction/2`, which re-presents
  once on `commit_unknown` (`:1752-1768`), so two calls — and then **a records
  load in the same callback**, the shape the acquisition path shows plainly
  where `transact_owner/1`'s commit at `:1363` runs on into
  `recover_committed_owner/1`'s loads at `:1397-1400` without returning.
  Three, and then the drain's own admission, which is **one** because it does
  not retry — see the non-retrying commit below. **Four, in sequence.**

  An earlier revision counted two and then three, both times by counting Store
  calls where the thing that blocks is a callback. It is the same error the
  create recount made from the other direction: a count taken from the shape
  of the code rather than from a run of it.
...
6. **The remaining composed edges stop, then the Store last.** Reverse start
   ...
   **What refuses its commit is the fence, not a dead Store**, and an earlier
   ...
   acceptable transaction. It is refused because **phase 4 has already moved
...
3. **A Store-linearized fence at the deadline, because terminating a
   ...
   **Phase 4 fences every session quiesce enumerated — settled, unsettled and
   absent alike — and an earlier revision fenced only the ones that had not
   ...
   enumerated session, concurrent, inside the same 90 s — the phase is three
...
| Phase 2, abort admission | a fixed **120 s** — four sequential Store calls at the adapter's own 30 s `@call_timeout`: the three of the deepest **callback** the coordinator may already be running — a commit that re-presents once, then a records load in the same callback — and then the drain's own admission, which does not retry; one phase for every session, because they are admitted concurrently | Core, against `apps/loopex_store_local/lib/loopex/store/local.ex:65`, `:111-113` and `session_coordinator.ex:146-149`, `:1363`, `:1397-1400`, `:1752-1768` |
...
| Maximum graceful stop | `budget_ms` + **555 s**, the sum of the seven fixed phases above — eight phases, of which only the drain budget is derived; a worst case built from the adapter's wedged-filesystem ceiling, not the cost of an ordinary stop | This plan, as the sum of its parts |
