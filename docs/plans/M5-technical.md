...
**And the single release run must be on Linux.** A cross-UID witness needs a
second unprivileged user and the `SO_PEERCRED` mechanism, neither of which the
Darwin lane supplies, so a release check run only on macOS would leave Outcome
2 unproved while reporting green. The closure matrix therefore states the
platform: `bash scripts/check-release.sh` runs once **on Linux**, and the
evidence page records the platform beside the result so a reviewer can see
which machine produced it. Closure is bound to that lane's recorded result and
to its platform.
