...
| `loopex_daemon` | Marker-first process and socket lifetime, the peer-credential check, generation-2 negotiation, per-connection socket output buffers, the resident window and aggregate byte ceiling, attachment residency and eviction, the in-memory controller lease and writer-epoch check, the session index and its bounded pages, session residency between active and dormant, and diagnostics | Store or coordinator internals, a second loop, policy selection, host identity, a durable record or a durable method |
...
| `loopex_llm_reqllm` | The credential handoff, its sender, the resolver behaviour it declares, the launch configuration that carries the composition-time environment snapshot, the `req_llm` version floor, the re-pointed credential-plane proofs and the provider suite's concurrency | Any host credential policy, any new credential scope, where a host keeps the bytes behind a reference, or anything outside the adapter |
...
session, which the holder may send before attaching. The lease record lives
...
| Idle time before eviction, and before an unused session goes dormant | 10 minutes | ADR 0032 |
| Active sessions per daemon | 64; a further activation refused while every slot is in use | ADR 0032 |
| Session index entries per root | 4,096; a root whose directory holds more refused at daemon start | ADR 0032 |
| `session.list` page | at most 256 entries, `limit` in 1 to 256 | ADR 0032 |
| Socket path bound | the platform's usable `sun_path`, one byte less than the structure because of the terminator: at most 103 bytes on Darwin and 107 on Linux, derived and tested per platform rather than assumed, refused at start as `socket_path_too_long` | ADR 0032 |
