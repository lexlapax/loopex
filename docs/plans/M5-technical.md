...
8. **In the already verified `0700` subdirectory, inspect the selected path
   ...
   the daemon's already verified effective uid, its `type` is `:other`, and
   ...
   follows. A predecessor's proved socket is cleaned up here rather than by
   itself.
...
- **Only the next verified placement-lock and marker holder may inspect and
  ...
  only a same-user pathname whose mode bits prove `S_IFSOCK`; every other
...
`SIGINT`, `SIGHUP` and `SIGQUIT` retain the same native-disposition
assertions as the daemon table and do not claim import cleanup.
...
- **Unsafe selected paths are preserved.** After both exclusions are held,
  ...
  `mode &&& 0o170000 == 0o140000` and the daemon uid, then proves only that
  path is removed and replaced by the serving socket. These cases prevent a
