...
resource admission or activation, prompt, optional follow-up or steer, or
takeover prompt retains its method, preallocated command ID, fixed input and
`not_sent | sent_unconfirmed` state, and the record retains `next_step`. A steer
also retains the `run_id` once `run.started` supplies it and fixes its canonical
input before send. The client changes a step to `sent_unconfirmed` before its
first socket-write attempt, so a failed write is treated conservatively as
possibly delivered. It advances `next_step` only after the accepted admission
or exact durable fact that makes the following step valid. It never retains a
writer epoch as reusable authority.
...
| A controller whose current planned step is `sent_unconfirmed` | Reconnect, reacquire and attach at the retained cursor, using dormant recovery only for `run` or `resume`, then observe durable replay for that exact command identity. Never resend it under the fresh epoch. An accepted admission or exact durable prerequisite advances to the retained next step; a refusal ends the plan; failure to obtain a decisive fact by the fixed deadline exits non-zero with that step unresolved. |
| `run --daemon` between prompt and optional follow-up or steer | A retained `not_sent` follow-up is sent once after the prompt's acceptance is proved. A retained steer waits for the exact replayed `run.started`, records its `run_id`, fixes the steer input and sends once. If the prompt is `sent_unconfirmed`, the client never substitutes the secondary command for it; it first follows the preceding row. |
...
| `attach --take-over` after its optional prompt reached `sent_unconfirmed` | Reacquire and attach without resuming and observe replay for that exact prompt; never resend it. A dormant result takes the same bounded release-and-exit path. |
...
| **CLI ↔ socket** | `loopex_cli` | Socket unreachable, refusal, transport loss, renewal failure, a complete `daemon.stopping` record or a handled client signal | The client retains its command form, durable create and resume identities, session ID, role and last emitted cursor, plus an exact ordered mutation plan. Every resource admission or activation, prompt, optional follow-up or steer, and takeover prompt has its method, preallocated command ID, fixed input and `not_sent | sent_unconfirmed` state; the record also carries `next_step`, and a steer carries the exact replayed `run_id` before its input is fixed. It never retains a writer epoch. List, status and liveness use one 10-second query clock and at most one reconnect/retry. Streaming uses one non-resetting 35-second recovery clock with 250 ms retry delay: a lost create reply replays the same command ID; run and resume may reacquire, attach, inspect and resume after restart; observers and takeover never activate. A `not_sent` next step is sent exactly once after recovery and proof of its prerequisite; a `sent_unconfirmed` step is never resent under the fresh epoch and advances only after its exact admission or durable fact. `control_held` retries only until the prior lease can have expired. A complete stop record is terminal; bare EOF follows the phase matrix. Handled live-client signals suppress reconnect and never send `session.abort`, with one bounded release attempt only when a lease is held and the socket remains writable | Exact compact JSON for list and status; contiguous at-least-once streaming from the retained cursor with seam duplicates removed; terminal stop reason, bounded refusal or non-zero unresolved outcome on `stderr`; signal exits fixed by the live-form table | **Durable:** nothing the client holds. The cursor and recovery phase are client-side positions; durable command identity makes create replay safe |
