...
| 6 | Provider child receives its credential over the bootstrap channel: the parent VM's environment holds no credential before, during or after a call (asserted on the parent and on a child probe); a child of one invocation cannot observe another invocation's credential when two run at once; the crash-report, late-delivery, sink-loss, rotation and version-refusal cases of `credential_plane_test.exs` pass against the channel handoff with their assertions unchanged in meaning; a security review of the handoff is recorded with the milestone; the provider suite runs its modules concurrently and its measured duration is recorded beside the M4-closure baseline (~250 s serial on the Mac) |
