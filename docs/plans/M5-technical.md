...
cases report `0`. This is abrupt BEAM shutdown before daemon ownership begins,
not the orderly sequence. No daemon resource has been acquired, and there is no
readiness line, wire record, `daemon.stopping` record or lifecycle/fatal
diagnostic.
...
service manager, both of which stop it with `SIGTERM`, and a daemon that also
handled direct `SIGHUP` would exit when the terminal that happened to start it
...
cases: the four launcher routes plus direct `SIGTERM` assert the five orderly outcomes, and
