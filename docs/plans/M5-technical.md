...
- **Tickets cover every core call that mutates, and nothing else — nine of
  them.** Eight are the lease-authorized existing-session mutations ADR 0033
  ...
  not keep. **`session.release_control` is not among the nine**, and the
...
- **A replacement owner's first grant for a session blocks until that
  ...
  ticket names no session and blocks no grant.
- **The admission cut is the relay's, because the relay is the only process
  that can answer for all nine.** The owner calls it once at the start of an
  orderly stop; it stops admitting, and it answers only when every ticket it
  holds has either settled or been recorded as outstanding for the drain to
  fence. That answer is what makes "after this instant nothing enters core
  through the daemon" a fact rather than a policy, and it is why every
  mutating call goes through this one process.
