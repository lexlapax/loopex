...
| --- | --- | --- |
| `loopex` | Durable session truth, the race-free attach barrier and cursor, independent concurrent attachments to one session, the read-only session-existence query, **`Loopex.Trace.exclude_self/1`, which keeps a credential-bearing process out of every trace session, live and future, before any message is delivered**, **the bounded `quiesce/2` that settles every active coordinator or names what it could not**, **the runtime-side create result's `disposition` and `residency` fields**, the per-attachment event-count dispatcher queues, cancellation and recovery | A lease, a transport, a byte limit, residency policy or any daemon fact |
...
journal and its format are unchanged, and its adapter is unchanged in behaviour, gaining one read-only call — whether the marker file still holds this daemon's marker — that writes nothing and that the socket-unlink precondition needs; the embedded API, public
...
milestone and states the rename itself. So ADR 0023 stays accepted and
unedited, and nothing here needs an amendment path.
...
What changes is its **name**, and the name is an input to
`LoopexProtocol.Session.schema_digest/0` — so generation 1's schema digest
changes with it, and so do the schema and vectors files that carry the string.
All of them are **re-pinned** in the same change; none of them is "preserved".
The plan says so wherever it used to say the generation-1 pins were untouched,
because a reader who trusts that sentence would read a failing build as drift.
...
`public_schema_conformance_test.exs` and `session_schema_test.exs`;
...
- **while running**, one per linked component in the fixed set: `store_lost`,
  ...
  `relay_lost` and `listener_lost`. A lease owner's death is **not** in this
  list: it is
...
binds `fresh?` at `control.ex:895`, and the `cond` at `:903-907` reads it to
decide whether to `start_owner`. What is discarded is the distinction in the
**reply** — all three branches answer `{:ok, session_id}` — so the fifth change
is to stop discarding it there. **Core's runtime-side create result — the
...
**It links a fixed set of nine processes — eight without artifact
...
| 8 | The **admission relay** | The daemon's own; every core call the daemon makes is routed through it | no |
...
| How many can exist at once? | One per session under lease or acquisition, capped at **512, the per-daemon attachment ceiling ADR 0032 already fixes** (`0032-…-technical.md:720`), reused rather than invented, with the cap itself stated at `0032-…-technical.md:222`; a lease operation that would exceed it refuses with `control_capacity_reached`. The population is **independent of the activation ceiling**, which counts something else entirely |
...
executor, workspace lease, transfers, custody, registry — and then **the
...
the fixed nine — eight without transfers — is `start_link`ed by the owner,
...
6. **The remaining composed edges stop, then the Store last.** Reverse start
   ...
   **Each of these four is a helper calling
   ...
   | Custody, then the registry | What remains of `teardown_ms` | Nothing durable |
   | **The Store** | A **fixed 30 s**, its own phase | Its `terminate/2`, which releases the writer marker (`local.ex:166`) |
...
**Three parts, and the first is the part an earlier revision got wrong.**

1. **A durable admission, before any cancellation.** ADR 0023 fixes that
   *only* the durable `session.abort` command cancels, and that cancellation
   is "never inferred from EOF or death"
   (`0023-…-technical.md:365`, with the method table at `:169`). Quiesce
   cancels, so quiesce must admit. For every active session it therefore
   **admits the existing abort command**, unchanged, and only then runs that
   session's cancellation. The
   path is the one that exists: `propose_new/3` for `%{type: :abort}` writes a
   `command_admitted` record with `"command_type" => "abort"` and
   `"admission" => "accepted"` (`session_state.ex:1684-1700`), resolving the
   queued steer and follow-up as it goes, and answers with
   `"admission" => "rejected_no_active_run"` where nothing is running
   (`:1750-1761`) — the right answer for an idle session, needing no special
   case.
...
2. **A derived budget.** `drain_deadline_ms` is `drain_ms`: the maximum over
   the drained sessions of `cancellation_bounds(g_i).cli_backstop_ms`
   (`apps/loopex/lib/loopex/executor.ex:456-474`). Inside it each session's
   cancellation runs under its own `g`, through `Loopex.Executor.cancel/4`
   (core) exactly as the coordinator already drives it at
   `session_coordinator.ex:3614` and `:5366`.

3. **A synchronous fence at the deadline.** When the budget is spent, core
   does not merely report what did not settle: for each unsettled session it
   **fences the coordinator, terminates it, and waits for its `DOWN` before
   returning**. Without that, "unsettled" would name a coordinator still
   running and free to commit a terminal fact after the daemon had reported it
   unfinished and moved on to tell clients — a report false by the time it was
   read. After `quiesce/2` returns, every session in `unsettled` has no live
   coordinator, and nothing can be appended for it until the next activation.
...
`custody_lost`, `relay_lost`, `listener_lost`), nothing stopped the Store, so the halt
...
- **In-flight shutdown, both halves of it.** Two cases, because the drain has
  ...
  *It does not.* A daemon with an effect held past the drain budget and an
  ...
  it, which is what the synchronous fence exists for; and then the negative
...
- **One class per linked component of the fixed set.** Each is
  ...
  opening that root with nothing to recover; eight of them in a daemon with
  transfers enabled and seven without — `runtime_lost`, `transfers_lost`,
  `workspace_lease_lost`, `executor_lost`, `registry_lost`, `custody_lost`,
  `relay_lost`, `listener_lost`, beside the two store classes. The set is closed for the
...
- **Owners exist for dormant sessions, and the population is not the
  ...
  count is unchanged**, and that after `session.release_control` — with no
  acquisition waiting and no ticket outstanding — the owner **retires**,
  proved by the daemon reporting one fewer live owner while the session stays
  reachable. A second case runs `loopex attach --take-over` against a dormant
  session, which acquires, is refused at attach, releases and exits, and
  asserts the owner that existed for the acquisition is gone afterwards and no
  activation was spent.
- **The concurrent-owner cap is the attachment ceiling, not a new number.** A
  daemon driven to 512 concurrent lease owners refuses the next lease
  operation with `control_capacity_reached`, and the case asserts no
  five-hundred-and-thirteenth owner was started and that releasing one lease
  lets the next acquisition through.
...
core concurrent-attachment change, **one process-level trace exclusion,
`Loopex.Trace.exclude_self/1`** — which unifies nothing and is not asked for
by this milestone's features at all: it exists because accepted ADR 0030
already requires that a key-bearing call be excluded before delivery, and the
implementation did not do it. Direct code cannot supply it, because exclusion
has to happen inside the tracer that owns the session. Two smaller forms were
tried and dropped: redaction at the sink is too late, the raw call having
already reached the tracer, and a list of excluded `{module, function, arity}`
identities cannot cover what an excluded function *calls* — a traced
`:gen_tcp.send/2` shows the credential frame however this adapter's own
modules are patterned. One pid excluded from every session, live and future,
is both smaller and total. **Two plain fields on the runtime-side create result** — which unify
...
| Attachments per daemon | 512 | ADR 0032, `:720` |
...
| Aggregate retained encoded events per daemon | 512 MiB | ADR 0032, `:147` and `:716` |
| Concurrent lease owners | 512, the attachment number reused rather than a second limit | ADR 0032, `:222` |
...
*attachments* per daemon (ADR 0032 `:720`), 512 *MiB* of retained encoded
events across every buffer and window (`:147`, `:716`), and at most 512
concurrent *lease owners* (`:222`), the last deliberately reusing the
