...
| `loopex_cli` | `loopex daemon` with its readiness line, signal handling and exit classes; `loopex attach` with its roles, takeover presentation and cursor reconnect; the live `run`, `resume` and `sessions` forms beside offline forms whose released grammar, session-driving semantics and output remain stable; and Outcome 6's governed host-side credential custody and registry composition | Normative lease or session semantics, or any other change to released offline command behavior |
...
| The integration scripts — `scripts/check-release.sh` and its fixtures | Workstream 5 replaces the shared real-provider application lanes with an eight-row fresh-BEAM manifest; the daemon workstreams add the daemon's real-provider, Node, long-bound and Linux cross-UID cases |
...
without calling back into `Control`. Trace alone retains the full handle from
`:trace.session_create/3`; it sends Control only the weak `{name, id}` element
split from that exact handle, never a name-selected result from
`:trace.session_info(:all)`. Control retains normalized configuration and
selected MFAs and monitors the exact Trace pid. Explicit stop removes the
retained selection only after Trace acknowledges destruction. That Trace pid's
...
Trace released the predecessor's sole strong handle when it died, so the weak
identity is already absent and no destroy is attempted. A defensive destroy
branch exists only for a present identity; its witness is an explicit
fault-injection fixture that retains one forbidden extra strong handle outside
`Control`, a state the production ownership graph cannot create. A racing
...
| --- | --- | --- | --- |
| Concurrent attachment (core change 1) | `apps/loopex/test/concurrent_attachments_test.exs` (**new**) | `several attachments under one holder coexist without replacement`; `attachments under different holders coexist`; `one detaching leaves every other attachment delivering`; `holder DOWN removes that holder's full attachment set from Control and the dispatcher and releases every transfer`; `the internal attach-for-holder call installs the supplied stable holder rather than its transient caller`; `a command executed by a relay task is admitted when its stable holder owns any live attachment for the pinned session`; `replace_attachment_id replaces only the named attachment owned by that holder and releases its transfers`; `an unknown, foreign or different-session replacement target is refused`; `an exact live replacement target is reserved once: exact repetition coalesces and a distinct concurrent request refuses without a second borrow`; `the replacement selector is part of repetition binding`; `Control allocates one attachment and incarnation identity at reservation and that exact pair appears in Control pending/live state, dispatcher staged/live state, the returned or repeated handle and cleanup acknowledgement`; `a Control generation change between dispatcher staging and validation discards an ordinary pending attach with both live maps equal`; `the same forced refusal for replacement at the 512 ceiling leaves the old attachment and transfers live, clears pending state and never undercharges the daemon count`; `post-publication holder death is forced in both Control mailbox orders for ordinary and replacement attaches: publish-ack then DOWN finalizes before ordinary cleanup, while DOWN then publish-ack returns holder_unavailable only after idempotent dispatcher cleanup; both leave equal live sets, zero pending rows and no retained charge, and neither restores a replacement target`; `a post-publication interruption reconciles by transaction identity before returning a result`; `the state-changing half is one deferred infinity Control call whose handle_call returns noreply, whose dispatcher acknowledgements go directly to Control, and whose only terminal reply comes from Control while post_commit remains responsive`; `caller death drops only the reply route and the transaction reaches cleanup`; `one linked preparation worker exists per pending attach, worker death refuses only that transaction, and 512 pending attaches create no 513th worker`; `EventDispatcher death before stage, after stage, after authorization and after publish acknowledgement was sent but before Control consumed it returns attachment_superseded to a live retained embedded caller within the fixed bound, while the daemon sentinel instead fail-stops runtime_lost and teardown settles its relay ticket and charge once; both paths clear predecessor live, pending, waiter, repetition, monitor and borrowed-charge state, treat late predecessor messages as cleanup-only, never restore a removed target, make old handles stale and permit a fresh attach after recovery`; `replacement registration seeds Control's retained acknowledged event sequence, so an unacknowledged outbox row stays fenced until resolution`; `a replacement stays mailbox-responsive while a linked registration worker talks to Control, accepts and max-merges an acknowledgement while Control is in post_commit, then opens admission only after dispatcher-ready and Control-ready states agree`; `dispatcher death kills its registration worker and no predecessor worker survives restart`; `one backpressuring attachment does not stall another`; `each attachment carries its own cursor and incarnation` | fast |
...
| **`Loopex.Trace.exclude_self/2`, `Control`'s excluded-pid/MFA state and `Entry`'s keyword-key redaction** (core change 3) | **`apps/loopex/test/trace_session_test.exs`** (existing; extended) | `the named MFAs produce no raw message under an explicitly named module, before any process flag is set`; `an excluded process drains its pre-clear trace signals through :trace.delivered before acknowledgement and produces no post-ack trace message`; `level :calls retains no pending start while return-bearing levels monitor each represented pid`; `managed exclusion purges the sender's pending-call keys at the delivery marker`; `managed and Direct sender DOWN return pending-call state and monitors to baseline`; `destroying a trace session removes that session's process flags and pending-call monitors`; `Trace alone retains each full session handle while Control retains the exact weak element split from that handle, with exactly one current Trace monitor and no start/status publication before replacement acknowledgement`; `the rerouted Runtime.trace/2, trace_stop/1 and trace_status/1 preserve their existing public result shapes`; `a tracer restart forces both mailbox orders of predecessor DOWN and replacement hello, keeps the replacement idle until Control resolves the exact predecessor, proves the ordinary production predecessor weak identity absent after Trace released its sole strong handle, exercises the present-and-destroyed branch only in a separate fault-injection fixture that deliberately retains one forbidden extra strong handle outside Control, and normalizes a racing false or badarg only after a fresh all-session query proves that exact identity absent, performs no lookup by the shared name, and creates a new session that reapplies retained live-sender pid and MFA exclusions`; `the last sender DOWN restores each live session's own selected MFA pattern while a session that never selected the MFA remains clear`; `a new session skips every already-excluded pid`; `fails closed while the capability or Control is immediately unavailable`; `an unanswered exclusion remains pending for the guardian deadline`; `a Control restart stops Trace, the sole full-handle owner, and downstream sender owners together, after which a fresh sender excludes anew`; `the excluded set and ref-counted MFA union return to baseline after the last sender exits`; `a keyword list's value is redacted under its own key`; `the same value under a key naming nothing is rendered, so the case cannot pass vacuously` | fast |
| **Managed provider lifetime** (core change 6) | **`apps/loopex/test/provider_lifetime_test.exs`** (new), plus **`apps/loopex_llm_reqllm/test/credential_plane_test.exs`** | `managed scope starts an inert guardian and an inert sender sequentially as direct temporary owner_workers children, with at most one linked start proxy alive`; `each proxy reports the exact start_proxy_result tuple for guardian or sender, reference, proxy pid and either the exact child pid or unavailable; the callback accepts only that matching result plus the exact child acknowledgement, monitors the child as soon as either message first discloses it, and consumes proxy normal DOWN before continuing, so no second proxy overlaps`; `the three legal start orders are child acknowledgement then result then proxy DOWN, result then child acknowledgement then proxy DOWN, and result then proxy DOWN then child acknowledgement; same-sender ordering forbids proxy DOWN before its result`; `a starter refusal and a caught starter exit follow that same unavailable result and reverse-clean an acknowledged child`; `the guardian initial argument is exactly guardian_ref, callback-owner pid and stop_reference`; `the sender initial argument is exactly sender_ref, callback-owner pid, guardian pid and tracing capability`; `suspending owner_workers before guardian materialization, ending the scope and resuming produces only an inert late guardian and every count returns to baseline`; `pausing guardian start acknowledgement before Core registration and killing the callback produces no registration or provider action`; `Core registration returns the exact retainer and cleanup grace, the guardian installs that monitor and acknowledges authorization before sender start`; `every private gate uses the exact tagged guardian-started, authorize-guardian, guardian-authorized, sender-started, authorize-sender, sender-adopt, sender-adopted-by-guardian and sender-adopted tuple with the expected reference and pid`; `after registration, holding sender materialization while Core sends its exact stop request yields the stop acknowledgement and guardian DOWN; resuming owner_workers creates only an inert late sender that sees callback or guardian loss and exits, with every population at baseline`; `sender start, guardian adoption and sender adoption-complete acknowledgement are separate cuts, and initialize cannot race either`; `guardian or sender death after either first pid disclosure and before transfer completion reaches the callback child monitor as unavailable; the callback demonitor-flushes only after the retainer monitor owns the guardian or the guardian monitor owns the sender`; `immediate refusal at any gate returns unavailable, reverse-cleans partial children and never raw-spawns`; `a non-answering start remains owner-group liveness rather than adapter timeout`; `normal callback completion after both transfers leaves the guardian retained and the sender governed by the guardian, not callback DOWN`; `the adopted sender parks until exact provider readiness, so pre-ready refusal or guardian death performs no exclusion, sink, route, custody or frame work`; `release, bootstrap result, credential context, every phase result and every continuation use the exact sender_ref/guardian-pid/sender-pid binding; phase continuations also carry the exact phase and unchanged deadline, and wrong-ref, wrong-pid, wrong-phase, changed-deadline and stale-replay injections never unpark, route or write and end at the unchanged deadline`; `managed setup consumes and never resets the absolute invocation deadline, release after 5 seconds but before that instant succeeds, and expiry after initialize returns timeout`; `before accepting provider readiness, the bootstrap result or any phase result, and before emitting credential context or a phase continuation, the guardian rechecks monotonic now before deadline; paired mailbox-order cases prove a completion consumed before the instant advances once, one consumed at or after it is cleanup-only timeout even when queued first, and the timer only prompts the check`; `after begin_bootstrap the sender keeps trap_exit false, completes exclusion before starting a sink that is linked for abnormal exit and monitors the sender for normal exit, and both pids are clear before token delivery`; `normal sender success and abnormal or kill exit each return the sink population to baseline`; `abnormal guardian death while the sender is parked or post-readiness sink installation is blocked ends sender and sink, restores Control and Trace baselines, and delivers no token, handle, channel context, frame or raw trace canary`; `normal guardian cleanup explicitly stops and awaits the sender`; `only the adapter-minted direct/no-runtime mode retains the existing raw-spawn fallback`; `killing Control while a sender is paused after custody reply delivers guardian and sender DOWN before the replacement tracer appears`; `a fresh sender succeeds after restart`; `ordinary completion leaves no guardian, sender, start proxy, receiver, generic phase-send helper, sink, Port, carrier, guard or provider BEAM behind, and every member's population returns to baseline`; `credential_plane_test.exs: credential frame result is emitted only from a non-secret final wait` proves all credential-bearing logical references are dropped before the exact `:credential_frame` result; `credential_plane_test.exs: only final-state normal sender DOWN starts invocation` proves the final continuation permits only normal exit and only that exact normal `DOWN` before the retained deadline starts the generic invocation helper; `credential_plane_test.exs: sender DOWN and final-deadline cuts never start invocation` forces raise, exit, kill and early normal exit before expiry to `:unavailable`, consumes the same `DOWN` after the guardian clock reaches the instant as `:timeout`, and expires the final continuation with no helper | fast |
...
that succeeds. Once a valid index exists, later switches need only stop the
...
- **the extracted-build witness separately compares a canonical manifest**,
  ...
  two to be identical. The manifest is one record per path, **NUL-separated**
  so no filename containing a newline or a quote can forge a boundary, holding
  the **path**, its **kind and mode** (regular with its permission bits,
  symbolic link, directory), and
  ...
  `mode` is the lowercase octal permission mode, `path` is relative to the
...
runs happen.** The tested implementation SHA already contains an indexed,
...
closure commit has the tested SHA as its sole parent; it fills that
...
observation with its conditions. It adds no heading, label, row or prose. It
...
row against the implemented release script. Fourth, the recomputed administrative archive matches every
non-documentation entry in the tested archive except root `README.md` and
`SOURCE_IDENTITY`. The README is validated by complete-patch confinement,
...
SHA-256 digest, both manifest references and digests, and both validated
...
socket or component work. There is no credential flag and no project-resource
flag: those inputs have the sources below.
...
project-resource or socket inputs because it composes none of those edges. The
`prepare-index` word must be the first argument after `daemon`; it may appear
...
never falls back to the environment. The daemon refuses with the typed class
and exact status below when an input is missing or unusable:
...
| Project resources | discovered from the selected workspace through `LoopexComposition.ResourcePacks`, as the app-server host does | an empty manifest when no `.agents/skills` exists | `project_resources_unusable` when workspace identity, discovery or manifest validation fails |
...
Every one of these is refused **before** the placement lock is acquired where that is
...
shutdown after readiness, whatever the sessions were doing. A successful `prepare-index` also exits `0`, writes nothing to
`stdout`, and never emits a readiness record; its ordinary progress and warnings
go only to `stderr`. An offline-command parser refusal uses the reference CLI's
existing status `1`, prints its bounded reason or usage on `stderr`, and performs
no placement or Store work. An operational offline-import refusal reuses the
...
classes are the fatal-class map's, in full, and this list is derived from it
...
- **during startup**, before the sentinel's exact `begin_accept` send: the
  ...
  `provider_credential_required`, `project_resources_unusable` and
...
- **during offline import**, which has no readiness or wire surface:
  ...
  placement identity — and `session_index_write_failed`;
...
loopex run --daemon <socket>
           [--steer <text> | --follow-up <text>]
           [--skill <name>]...
           [--skill-resource <name>:<label>]... <prompt>
...
| --- | --- | --- | --- |
| `run --daemon` | `--daemon`; optional `--steer` or `--follow-up`; repeatable `--skill` and `--skill-resource` | `--policy`, `--state-root`, `--workspace`, `--cleanup-grace-ms`, `--context-token-budget` | Steer, follow-up and resource selection are request choices. Skills are resolved through `resources.catalog` and `resources.read`, then admitted and activated through the generation-2 resource methods. The daemon owns policy, root, workspace and runtime configuration. The existing steer/follow-up mutual exclusion and skill/resource validation apply unchanged |
...
- **`loopex run --daemon <socket> "<prompt>"`** — creates a session and sends
  ...
  `session.acquire_control`, **`session.attach`**, then, when project
  resources exist, the released resource-admission flow against the daemon's
  catalog and every selected skill activation, and only then the prompt under
  the granted writer epoch. Attach comes *before* the prompt because
...
- **`loopex resume --daemon <socket> <session-id>`** — reaches an existing
  ...
  succeeds, the session is already active and the client streams without
  sending a redundant `session.resume`. When attach refuses
  ...
  **fresh resume command ID** and the granted epoch, retries attach exactly
  once, then streams. Thus an active session follows acquire → attach, while a
  dormant session follows acquire → refused attach → resume → attach; only the
  resume branch activates. If core's temporary coordinator died, this daemon's
  one-way activation set still makes the first attach succeed; the first
  session-driving mutation then returns core's unchanged
  `session_unavailable`, and repeating the command against the same daemon
  repeats it. The client exits non-zero and names the exact remedy: restart the
...
If the session went dormant in between — because the daemon restarted — its
first attach after acquisition returns `session_dormant`; the client then
resumes with a fresh resume command ID under that new epoch and retries attach
once. That is the same verified-dormant exception ADR 0033 fixes, reached from
...
| `loopex run --daemon`, `loopex resume --daemon`, the create-and-drive sequence, and the restart-then-resume remedy after a temporary coordinator dies | `docs/operator/daemon.md`, driving section |
...
6. **Create or verify the owner-only `0700`, non-symlink `daemon/` directory,
   then load the daemon's versioned index**, reading at most its fixed
   encoded-file ceiling, validating its version, digest, unique sorted rows and
...
8. **In the already verified `0700` subdirectory, remove any stale socket
   pathname and bind the `0600` socket**, then read back and verify ownership and mode.
   Removing the stale pathname is this daemon's right and only this daemon's:
   it holds both the verified placement lock and Store marker, which is the
   only moment at which removing a socket file is unambiguously correct. A predecessor that left one — every
...
`stderr`, and a raised exit is caught into the same step family together with
the exact partial pid inventory already acquired.
...
- **The sentinel forwards startup stops to the owner until readiness begins,
  ...
  of the relay and connection-registry starts; before unlinking a stale socket
  pathname and binding the new socket; before starting the parked listener;
...
- **Only the next verified placement-lock and marker holder removes it**, at startup, before it
...
The witnesses are **one per signal per route** — eight route cases: the four
launcher routes plus direct `SIGTERM` assert the five orderly outcomes, and
direct `SIGINT`, `SIGHUP` and `SIGQUIT` assert the three documented
emulator/native dispositions. The direct-`SIGINT` case asserts only that the
...
An acquire or release whose `owner_lost` disposition won before a release
proposal instead uses
`{:settling_owner_loss, permit_id, class, actor_pid, actor_incarnation,
op_ref}` and requires no release record. The immutable actor binding and,
...
is idempotent and the mirror must be absent before core quiesce. For a fresh acquire,
whose actor is the daemon owner, that owner survives and kills and reaps only
the exact provisional child named by the returned `start_op_ref`, pid and
incarnation, first resolving any installed provisional mirror cancelled and
requiring its absence before quiesce. It never infers actors by scanning its
...
kills and reaps the exact fresh child. A `settling_owner_loss` descriptor needs no release record and
completes its retained exact `DOWN`, pop and classification join without client
output. Missing or mismatched required release state, malformed relay answers or
...
1. **The relay closes admissions first; then the transport stops accepting —
   ...
   **At the wait's bound the stop proceeds**, and what that costs is stated. A
   ...
   promoted ticketed mutation remains on its real-result path; a fresh acquire leaves the daemon
   owner alive and kills and reaps only the provisional child named by its
   `start_op_ref`, first cancelling and clearing any installed provisional row.
   ...
   pop/classification. A separately tagged pre-proposal owner-loss row completes
   the same retained `DOWN`/pop join without requiring a release record, all
   under the same freeze deadline. The exact
...
1. **The admission barrier and abort admission: core closes new routes, then
   ...
   minimum work allowance derives from the maximum drain-only path: two sequential
   local-Store calls at the adapter's 30,000 ms call timeout, with the drained
   abort presented once and never retried. The final 5,000 ms are reserved to
...
5. **The socket path is left alone**, exactly as the orderly path leaves it.
   ...
   the next verified placement-lock and marker holder removes it before binding. **The runtime root, the lease owners, the registry, the
...
same map directly in the command process. Offline import likewise maps its
post-parser state-root, placement, Store and index failures here without
starting a daemon owner. The classifier preserves the typed
...
| `state_root_unusable` | 66 | Startup or offline-import input: the state root cannot be created, resolved or read | — |
...
| `project_resources_unusable` | 74 | Startup input: workspace project-resource discovery or validation failed; an empty resource set remains valid | — |
...
| `session_index_corrupt` | 84 | Startup or offline import: the index version, digest, ordering, uniqueness or row shape is invalid, or canonical and legacy rows disagree on placement identity | — |
...
| `socket_permission_unverified` | 88 | Startup: subdirectory or socket ownership/mode could not be verified | — |
| `invalid_socket_path` | 89 | Startup: a `--socket` override is outside the selected root's `daemon/` directory | — |
| `signal_install_failed` | 90 | Startup: the required `SIGTERM` handler could not be installed | — |
...
| `relay_lost` | 105 | The admission relay exited, taking every outstanding admission ticket with it | `fatal:relay_lost` |
| `connections_lost` | 106 | The connection registry exited, taking every connection's slot, monitor and buffer control with it | — the record would have gone through the interface that is gone |
...
The mapping is injective and closed: status `0` has no fatal class; every
composition input, startup step, linked running component, drain failure and
owner loss has exactly one row; no atom is ever passed to `System.halt/1`.
...
| **After a Store pid is returned but before a successful bind** — later composition, index, relay/registry, subdirectory/socket creation, or an unsuccessful bind/listen | None | Removed **only if this start created it and it is empty**; a pre-existing or non-empty directory is left as found | Completed Store `terminate/2` invokes best-effort release. The healthy witness proves absence; an expired stop or ignored removal/sync error may leave the complete marker for stale recovery |
...
placement-lock and marker holder removes only the stale socket before binding;
...
| `Control`'s excluded-pid/MFA set and exact Trace monitor/weak session identity, plus `Entry`'s redaction of a keyword pair under its own key | Core change 3 | The trace-exclusion and tracer-replacement cases, and the render pair that must differ |
...
- **No daemon ever unlinks, and a successor's socket survives.** Three cases,
  ...
  final halt. A second daemon acquires the placement lock and marker, removes the stale path, binds and
...
- **Signals, one case per signal per route, and the route is what decides.**
  ...
  checked claim rather than a preference. The direct-`SIGINT` case asserts only
  ...
  point of the pair. A ninth asserts the install order at the exact
...
- **One millisecond remaining on the teardown clock exercises the probe's real
  call path.** The clock is controlled so a component stop receives exactly
...
- **Reverse cleanup, on both sides of the bind.** On the healthy cleanup path, a startup made to fail
  **before** the bind — at the index bound, after the marker is acquired —
  proves marker absence, **no socket pathname** and no `daemon/` subdirectory
...
| **Attachment preparation workers** | The current EventDispatcher starts one linked, monitored, non-trapping worker after Control reserves an attachment transaction | One per pending attachment; core's pending-row ceiling and the daemon charge bound both cap the group at 512 | One worker's abnormal exit refuses only its transaction. Dispatcher death ends the entire group; Control's generation cut selects `attachment_superseded` for live embedded callers, while the daemon sentinel independently fail-stops as `runtime_lost` and clears relay charges during teardown |
...
| **CLI ↔ socket** | `loopex_cli` | Socket unreachable, refusal, transport loss, renewal failure | The client's own reconnect loop and its renewal timer | Reconnect at the retained cursor, deduplicating. A correlated renewal refusal on a live socket drops to observer on the existing attachment. A failed send or EOF has already ended that attachment, so the client reconnects and reattaches at the cursor as an observer; a controller that intends to continue controlling must instead acquire again for a fresh epoch. Every branch reports the loss on `stderr` and sends no mutation under the old tenure | **Durable:** nothing the client holds. The cursor is a client-side position |
| **Daemon ↔ OS: signals** | The operator | `SIGTERM`, delivered to the handler the daemon installs before it takes the placement lock; a terminal `SIGINT` reaches it only as the `SIGTERM` the launcher forwards, since `:os.set_signal/2` refuses `:sigint` | The lifecycle sentinel records and forwards pre-readiness stops, arbitrates stops during readiness, and forwards post-release stops. The owner first completes the relay admission cut, the registry's `transport_closing` gate, the listener's untrappable kill and exact linked `EXIT`, and the provisional/uninitialized sweep under the one transport-cut deadline, leaving the socket pathname for the next verified placement-lock and marker holder. It then runs the bounded admission wait and core drain, tells initialized clients and closes every remaining connection, and stops the connection registry, lease owners, relay, runtime, edges, and Store in that order, with Store last in its own fixed 30 s phase — each process stop driven by a monitored helper while the owner waits on its own link until the shared teardown deadline and kills on expiry. It then runs the exact-handle placement attempt in its separate five-second helper. Classification uses the observed exit plus the exact `stopping` mode: `:normal` or `:shutdown` is consumed for `normal_stop`; the listener's exact owner-issued `:killed` is consumed for `planned_transport_kill`; and a timeout-issued `:killed` is cleanup-only after that target's fatal class was latched, so it can never independently permit exit `0`. Every other exit is classified | `daemon.stopping` with `operator_stop`, then close | **Durable:** whatever committed. **Not:** work ended crash-equivalently — a claim about the journal, not about every process being gone |
...
| **Daemon ↔ OS: socket file** | `loopex_daemon` | A stale `daemon.sock` left by any exit path | Only a daemon that has acquired and verified both the host placement lock and Store marker removes it, at startup, before binding; no daemon removes one on its way out, so no predecessor can delete a successor's socket | The loser of two simultaneous starts exits without touching the socket; a client meeting a stale path is refused rather than hung | **Not durable:** the socket file is a path, never state |
