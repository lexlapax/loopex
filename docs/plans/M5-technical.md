...
on its `DOWN`, releasing that attachment's transfers, and `Control` scopes
its own attachment and repetition state by that holder pid rather than
keeping one slot per session — because the first half
