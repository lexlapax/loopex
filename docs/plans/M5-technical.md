...
command input. If the first write reached core, command replay returns the same
accepted or refused admission; if it did not, core admits the re-presented
command once. Neither branch duplicates the mutation. The client advances `next_step`
