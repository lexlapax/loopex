...
The script already pipes each lane through `scripts/suite-summary.sh` and
fails the check when a lane executed no test — "a run that executed nothing
is not a pass", as its own comment puts it — and that judge governs this lane
too: **a zero executed count is red**, not a skip. That matters more here than
elsewhere, because a cross-UID case is exactly the kind that quietly excludes
itself when the second user is missing, and a silent zero would turn the one
witness the fast check cannot supply into no witness at all. Closure is bound
to that lane's recorded result.

`loopex_daemon` joins the `release_apps` list in the change that adds its
first release case, and only that list: every `release_apps` lane already runs
