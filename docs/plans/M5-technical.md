...
3. **Terminate every coordinator, then fence every session — two phases, in
   ...
   **One residual survives the fresh enumeration, and it is stated rather than
   closed.** A `session.create` whose ticket was still outstanding when phase
   1b proceeded may not have an entry even at phase 4's enumeration:
   `Control.create_session/2` makes up to **three** Store calls — the
   `runtime_command` at `control.ex:959-965` and the genesis commit at
   `:896-897`, re-presented once — before `start_owner/4` reaches
   `await_owner/9` (`:1176-1195`), which is where the `state.sessions` entry
   is written. A create inside those calls is invisible to any enumeration,
   so it is neither terminated nor fenced, and its coordinator may start and
   commit after `quiesce/1` has returned, while phase 7 still holds the Store
   open.

   What that leaves is bounded and already covered. Bounded: the create's own
   remaining depth, the rest of the ten calls phase 1b counts. Covered:
   whatever it commits is an ordinary genesis and owner advance for a session
   the daemon never indexed and never told a client about — which is exactly
   the crash cut ADR 0032's *recovery procedure for the crash cut* answers,
   where a client that never received the session ID replays its
   `session.create` under the same `command_id` and the daemon repairs the
   directory entry and index row. The next daemon meets a durable session it
   can reach by command identity, not a lost one, and no false terminal is
   claimed for it.
   ...
   **The classification quiesce returns has three lists, not two, and it is
   ...
   | `:awaiting_owner_barrier` | A coordinator has died with its **owner group still alive** and a caller still waiting (`control.ex:835`, `:1040`, `:1046-1053`, consumed at `:833-857`) — reachable exactly when phase 1b proceeds with a resume in flight | `unsettled`; no abort is admitted, there being no ready coordinator to admit one | **Yes**. Its coordinator is already gone, and the owner group that outlived it ends with the runtime kill in phase 6 |
