...
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 candidate](roadmap.md#concept-roadmap-session-protocol) | Core durable interactions and bounded ArtifactStore ranges; floor refresh across all actual Closed holders; foreground stdio JSONL and exact schema/capability negotiation; facade-only mapping of shared resources, interactions and commands; TypeScript skill/approval/tool/artifact/restart workflow; independent vectors, malformed input and slow-reader proof. | No public freeze; source 0.1.0 and any tag/publication require their separate decisions. |
...
| Bounded artifact retrieval and floor refresh | `M4` | ADRs 0028/0026 settle range integrity/work budgets and the protocol codec floor; all affected Closed holders retain transactions. |
