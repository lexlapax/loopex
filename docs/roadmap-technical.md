...
| <a id="technical-roadmap-useful-local-kernel"></a>[Useful local kernel — v0.1 candidate](roadmap.md#concept-roadmap-useful-local-kernel) | Three milestones over the durable single-machine kernel M1 delivered: a loop a developer uses through the embedded API and reference client, a foundation milestone that adds project skills and closes core repair debt, then durable interactions, artifact transfers and the language-neutral boundary an independent program drives. Restart and replay remain the continuity mechanism across all three. | No public freeze; surfaces remain experimental. |
| <a id="technical-roadmap-foreground-harness"></a>[Foreground coding harness — M2 candidate](roadmap.md#concept-roadmap-foreground-harness) | Multi-turn loop over committed conversation history with real tool results returned to the model; runtime-scoped tool registry with recorded definition generations; the `read`, `write`, `edit`, and `bash` bootstrap profile under one shared conformance suite; host policy that refuses before any process starts; truthful same-process cancellation; streaming progress; session discovery and resume; a one-page embedded composition; a `loopex` terminal command; an attended real-provider coding demonstration. Closed M2 and its integrated post-closure repair line supply this rung's source proof. | The annotated `v0.0.0-m2` tag identifies that exact integrated repaired source snapshot. It publishes no package, freezes no surface, and makes no compatibility claim. |
| <a id="technical-roadmap-kernel-consolidation"></a>[Extensible local foundations — M3 closed](roadmap.md#concept-roadmap-kernel-consolidation) | Closed and integrated proof: pinned Git/project-only skills, operator pre-run admission/selection and unchanged authority; required-only admission, dispatcher availability and permit retirement; skill/tool/artifact workflow, stable witnesses and acyclic inherited enforcement. | Experimental local additions; no release or freeze. |
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 closed](roadmap.md#concept-roadmap-session-protocol) | Observability first: runtime-owned isolated OTP trace sessions with identity-only default capture and telemetry spans at every port and transaction boundary; core durable interactions and bounded ArtifactStore transfers; floor refresh across all actual Closed holders; foreground stdio JSONL and exact schema/capability negotiation; facade-only mapping of shared resources, interactions and commands; a plain-JavaScript skill/interaction/tool/artifact/restart workflow from a fresh source extraction with operator-supplied inputs; independent vectors, malformed input and slow-reader proof. The [Closed M4 plan](plans/M4.md#concept) records the outcomes and their evidence; the runs are in [M4 closure runs](evidence/M4-closure-runs.md). | Source `VERSION` is `0.1.0`; the annotated `v0.1.0` tag identifies the exact integration commit on `main`. It publishes no Hex package, binary, installer or public freeze. |
| <a id="technical-roadmap-durable-service"></a>[Durable service — v0.2 candidate](roadmap.md#concept-roadmap-durable-service) | M5: daemon-owned session lifetime, Unix-domain-socket transport, concurrent independent clients, one-controller/many-observer collaboration with stale-controller fencing and crash takeover, race-free snapshot and cursor replay, backpressure, residency and eviction, cross-process cancellation, and provider-credential resolution moved from the parent VM's process-wide environment to a composition-bound token resolved per invocation through host custody before the unchanged private-channel frame. The store is ADR-selected: the existing local adapter with documented limits, the daemon-grade adapter and its migration deferred to the successor. | No public freeze; the protocol remains experimental. |
...
implementation-blocking prerequisite must be accepted before the plan pair is
accepted or the implementation that depends on it begins.

| Decision | Before | Why it blocks |
| --- | --- | --- |
| Repository and application layout | `M0` acceptance | Dependency direction needs an accepted tree. ADR 0001 makes the first scaffold responsible for creating repository-owned enforcement and an adversarial illegal-reference fixture; the Claude hook remains early feedback over it. |
| Runtime floor and exact version pairs | `M0` acceptance | The toolchain pairs had to be named before experiments could produce comparable evidence; M0 closure migrated repository checks and tested client-hook paths off the temporary Python/`jq` bridge. |
...
| Resource packs, optional-context shape and trust | `M3` | ADR 0025 extends the fixed stage and its admission proof; resources remain separate from VM-global activation and tool-output artifacts. |
| Safe provider-permit retirement | `M3` | ADR 0027 preserves fencing while retiring settled references. |
| Bounded artifact retrieval and floor refresh | `M4` | ADRs 0028/0026 settle transfer integrity/work budgets and the protocol codec floor; all affected Closed holders retain transactions. |
...
| Durable interaction identity | `M4`, core before wire | ADR 0024 assigns session-owned identity and answer/policy/intent order, proved locally before protocol mapping. |
| Store selection and migrations | `M5` selects, the successor migrates | A service makes operational and compatibility claims that the bootstrap adapters do not. ADR 0031 records the `0.2` selection and its limits; the daemon-grade engine and its migration wait for the experiments that choose them. |
...
| Provider credential handoff | `M5` | ADR 0034 moves parent-side resolution from a process-wide environment read to a composition-bound opaque token resolved per invocation through host custody; the credential still crosses the private channel ADR 0019 already provides. The global environment read is both the weaker property and what keeps the provider suite serial. |
