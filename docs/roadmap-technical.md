...
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 candidate](roadmap.md#concept-roadmap-session-protocol) | Observability first: runtime-owned isolated OTP trace sessions with identity-only default capture and telemetry spans at every port and transaction boundary; core durable interactions and bounded ArtifactStore transfers; floor refresh across all actual Closed holders; foreground stdio JSONL and exact schema/capability negotiation; facade-only mapping of shared resources, interactions and commands; TypeScript skill/approval/tool/artifact/restart workflow; independent vectors, malformed input and slow-reader proof. | No public freeze; source 0.1.0 and any tag/publication require their separate decisions. |
