...
| <a id="technical-roadmap-kernel-consolidation"></a>[Extensible local foundations — M3 candidate](roadmap.md#concept-roadmap-kernel-consolidation) | Compatible resource acquisition, content-bound trust and progressive skill use; durable host-policy interactions; bounded resource/artifact queries; required-only admission, asynchronous dispatcher reads and safe permit retirement; early real workflow, integrated repair protection and acyclic gate enforcement. Floor and Closed-gate prerequisites settle before M3 acceptance. | Experimental local additions; no release or freeze. |
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 candidate](roadmap.md#concept-roadmap-session-protocol) | Foreground stdio JSONL; exact schema/capability negotiation; facade-only mapping of M3 resources, interactions, commands and artifact reads; TypeScript skill/approval/tool/artifact/restart workflow; independent vectors, malformed input and slow-reader proof. | No public freeze; source 0.1.0 and any tag/publication require their separate decisions. |
...
| Safe provider-permit retirement and bounded artifact retrieval | `M3` | ADRs 0027/0028 settle fencing and integrity before memory/range optimizations. |
...
| Durable interaction identity | `M3`, before M4 projects it | ADR 0024 assigns session-owned identity and answer/policy/intent order before either terminal or external client depends on it. |
