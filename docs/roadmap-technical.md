...
| <a id="technical-roadmap-kernel-consolidation"></a>[Extensible local foundations — M3 candidate](roadmap.md#concept-roadmap-kernel-consolidation) | Pinned Git/project-only skills, operator pre-run admission/selection and unchanged authority; required-only admission, dispatcher availability and permit retirement; early skill/tool/artifact workflow, stable witnesses, focused checkpoints and acyclic inherited enforcement. Two new prerequisite ADRs; no planned pre-acceptance floor transaction. | Experimental local additions; no release or freeze. |
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 candidate](roadmap.md#concept-roadmap-session-protocol) | Observability first: runtime-owned isolated OTP trace sessions with identity-only default capture and telemetry spans at every port and transaction boundary; core durable interactions and bounded ArtifactStore transfers; floor refresh across all actual Closed holders; foreground stdio JSONL and exact schema/capability negotiation; facade-only mapping of shared resources, interactions and commands; TypeScript skill/interaction/tool/artifact/restart workflow from a fresh source extraction with operator-supplied inputs; independent vectors, malformed input and slow-reader proof. | Source-only `0.1.0` release and annotated `v0.1.0` tag after reviewed integrated closure and separate release authority; no Hex package, binary, installer or public freeze. |
