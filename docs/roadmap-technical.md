...
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 closed](roadmap.md#concept-roadmap-session-protocol) | Observability first: runtime-owned isolated OTP trace sessions with identity-only default capture and telemetry spans at every port and transaction boundary; core durable interactions and bounded ArtifactStore transfers; floor refresh across all actual Closed holders; foreground stdio JSONL and exact schema/capability negotiation; facade-only mapping of shared resources, interactions and commands; a plain-JavaScript skill/interaction/tool/artifact/restart workflow from a fresh source extraction with operator-supplied inputs; independent vectors, malformed input and slow-reader proof. The [Closed M4 plan](plans/M4.md#concept) records the outcomes and their evidence; the runs are in [M4 closure runs](evidence/M4-closure-runs.md). | Source `VERSION` is `0.1.0`; the annotated `v0.1.0` tag on the exact integration commit on `main` awaits separate release authority. No Hex package, binary, installer or public freeze. |
| <a id="technical-roadmap-durable-service"></a>[Durable service — v0.2 candidate](roadmap.md#concept-roadmap-durable-service) | M5: daemon-owned session lifetime, Unix-domain-socket transport, concurrent independent clients, one-controller/many-observer collaboration with stale-controller fencing and crash takeover, race-free snapshot and cursor replay, backpressure, residency and eviction, cross-process cancellation, and the provider credential handed over the child's private channel instead of the parent VM's environment. The store is ADR-selected: the existing local adapter with documented limits, the daemon-grade adapter and its migration deferred to the successor. | No public freeze; the protocol remains experimental. |
...
| Provider credential handoff | `M5` | ADR 0034 makes the credential a per-invocation input on the child's private channel; a process-wide environment read is both the weaker property and what keeps the provider suite serial. |
