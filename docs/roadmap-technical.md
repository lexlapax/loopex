...
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 accepted candidate](roadmap.md#concept-roadmap-session-protocol) | Observability first: runtime-owned isolated OTP trace sessions with identity-only default capture and telemetry spans at every port and transaction boundary; core durable interactions and bounded ArtifactStore transfers; floor refresh across all actual Closed holders; foreground stdio JSONL and exact schema/capability negotiation; facade-only mapping of shared resources, interactions and commands; TypeScript skill/interaction/tool/artifact/restart workflow from a fresh source extraction with operator-supplied inputs; independent vectors, malformed input and slow-reader proof. The accepted [M4 plan](plans/M4.md#concept) and [gate](plans/M4-gate.md) define the acceptance prerequisites and evidence; implementation proceeds on its milestone branch. | Source-only `0.1.0` release and annotated `v0.1.0` tag on the exact integration commit on `main` after reviewed closure and separate release authority; no Hex package, binary, installer or public freeze. |
| <a id="technical-roadmap-durable-service"></a>[Durable service — v0.2 candidate](roadmap.md#concept-roadmap-durable-service) | M5: daemon-owned session lifetime, Unix-domain-socket transport, concurrent independent clients, one-controller/many-observer collaboration with stale-controller fencing and crash takeover, race-free at-least-once snapshot and cursor replay, backpressure, residency and eviction, cross-process cancellation, all on the local store ADR 0031 selects for the release with its documented limits; the daemon-grade adapter with migration follows in a successor milestone under the same ADR. The [Open M5 plan](plans/M5.md#concept) and [gate](plans/M5-gate.md) are the planning lookahead for this rung; they accept nothing. | Source-only `0.2.0` release and annotated `v0.2.0` tag on the exact integration commit on `main` after reviewed closure and separate release authority; no public freeze, and the protocol remains experimental. |
...
| Store selection and migrations | `M5` selects; the successor migrates | A service makes operational claims that the bootstrap adapters do not, so the daemon's store is ADR-selected even when the selection is the existing local adapter with its documented limits; the daemon-grade adapter and its migration follow when a root outgrows those limits. |
