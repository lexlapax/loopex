...
| <a id="technical-roadmap-session-protocol"></a>[Headless external consumption — M4 open candidate](roadmap.md#concept-roadmap-session-protocol) | Observability first: runtime-owned isolated OTP trace sessions with identity-only default capture and telemetry spans at every port and transaction boundary; core durable interactions and bounded ArtifactStore transfers; floor refresh across all actual Closed holders; foreground stdio JSONL and exact schema/capability negotiation; facade-only mapping of shared resources, interactions and commands; TypeScript skill/interaction/tool/artifact/restart workflow from a fresh source extraction with operator-supplied inputs; independent vectors, malformed input and slow-reader proof. The [Open M4 plan](plans/M4.md#concept) and [gate](plans/M4-gate.md) define the acceptance prerequisites and evidence. | Source-only `0.1.0` release and annotated `v0.1.0` tag on the exact integration commit on `main` after reviewed closure and separate release authority; no Hex package, binary, installer or public freeze. |
