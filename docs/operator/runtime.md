...
1. Reopen the local Store with `recover_stale_writer: true`. The option is a
   ...
   alive: a live holder is refused as `store_writer_active`, except when the
   recorded OS identity is this VM's own and its recorded BEAM Store process is
   proved dead. That same-VM stale writer is recoverable without removing the
   marker by hand; an OS-process liveness sample alone would miss it. Otherwise,
   ...
   OS-process absence. A store that cannot record its own identity refuses to open, so
