...
and independent audits whose regression tests no accepted gate yet locks. The
projected shape is those tests locked, the recorded debt implemented rather
than carried, the closed gates run by something instead of by a person
remembering, and the runtime floor decided while it is still cheap to decide.
It ships no new operator or public surface, which is the point: a surface built
on unproved foundations makes every later defect twice as expensive to find.
