# Concept: M5 routing reuses the bound M3 support machinery for everything that
...
# M5 labels live here. This module never executes a gate or reads provider input.
...
defmodule Loopex.M5Gate.Support do
  ...
  def manifest(root) do
    {data, _} = Code.eval_file(Path.join(root, "scripts/m5-outcomes.exs"))

    ensure(
      is_map(data) and Enum.sort(Map.keys(data)) == [:outcomes, :real],
      "invalid M5 outcome manifest"
    )
  ...
  # against the complete M5 schema for its kind.
  ...
  def verify_final_report(line) do
    ...
    ensure(prefix == "LOOPEX_M5_GATE_REPORT", "final report prefix is not the M5 gate report")
  ...
  def main(args) do
    case args do
      ...
      ["identity", root, mode] when mode in ["committed", "working"] ->
        ...
        IO.puts(
          "LOOPEX_M5_SOURCE sha=#{sha} working_digest=sha256:#{digest} seed=3107 elixir=#{System.version()} otp=#{exact_otp_version()} erts=#{:erlang.system_info(:version)} platform=#{:erlang.system_info(:system_architecture)}"
        )
      ...
      ["build", build] ->
        IO.puts("LOOPEX_M5_BUILD digest=sha256:#{Shared.build_digest(build)}")
      ...
      ["selector-account", expected_path, observed_path] ->
        ...
        IO.puts(
          "LOOPEX_M5_SELECTOR_ACCOUNT expected=#{expected_count} observed=#{observed_count} result=PASS"
        )

      _ ->
        raise ArgumentError, "invalid M5 support command"
    end
  rescue
    error ->
      IO.puts(:stderr, "M5 gate UNAVAILABLE: #{Exception.message(error)}")
      System.halt(2)
  end
...
case System.argv() do
  ["--m5-gate-support" | args] -> Loopex.M5Gate.Support.main(args)
  _ -> :ok
end
