...
# still holds, the retained evidence has the required structure, and real user
# state was contained. It does not judge whether a test asserts what it names or
# whether an attended demonstration was a genuine task. Independent review owns
# those judgments and the gate does not pretend otherwise.
#
# TECHNICAL DEPTH
#
...
readonly DECLARED_RED="an operator cannot run a coding task from the command line; the session loop still stops after two turns, sends the model no conversation history, and offers only two demonstration tools"
...
require_feature \
  ...
  "a bounded turn ceiling ends the run truthfully instead of stopping silently" \
...
require_feature \
  ...
  "tool output beyond its declared bound spills to an artifact" \
  "a tool child process tree is owned and terminated with its job"

require_feature \
  "host policy cannot refuse a tool call; there is no working deny path" \
...
require_feature \
  ...
  "loopex run submits a prompt and prints the streamed answer with its tool calls and results" \
  "loopex sessions lists the operator's sessions and loopex resume continues one" \
  "loopex cancel stops the running task and prints what was observed" \
...
require_feature \
  ...
  "one page of shipped code starts a runtime creates a session submits a prompt and consumes events" \
...
require_feature \
  ...
  "one real provider task edits a real repository across several turns and the operator sees the committed result"
...
# an immediate end of file means no key, which fails the real-provider role
# rather than skipping it. The key is held in one unexported variable and is
...
[ "$version_reported" = "0.1.0" ] \
  || fail "the version train reports ${version_reported:-nothing}, not the accepted 0.1.0"
...
run_selector() {
  ...
  if [ "$role_name" = real ]; then
    real_path_record="$marker"
  fi
}

real_path_record=""

run_selector 1 apps/loopex/test/agent_loop_test.exs default 5 zero \
  ...
  "passed=a bounded turn ceiling ends the run truthfully instead of stopping silently" \
  "passed=each turn dispatches exactly the canonical request bytes and digest committed before it"

run_selector 2 apps/loopex/test/tool_registry_test.exs default 4 zero \
...
run_selector 3 apps/loopex_executor_local/test/coding_tools_test.exs default 7 zero \
  ...
  "passed=tool output beyond its declared bound spills to an artifact" \
  "passed=a tool child process tree is owned and terminated with its job"

run_selector 4 apps/loopex_executor_local/test/host_policy_test.exs default 4 zero \
...
run_selector 5 apps/loopex/test/cancellation_test.exs default 5 zero \
...
run_selector 6 apps/loopex/test/session_directory_test.exs default 5 zero \
...
run_selector 7 apps/loopex_cli/test/cli_test.exs default 6 zero \
  "passed=loopex run submits a prompt and prints the streamed answer with its tool calls and results" \
  "passed=loopex sessions lists the operator's sessions and loopex resume continues one" \
  "passed=loopex cancel stops the running task and prints what was observed" \
...
run_selector 8 apps/loopex_cli/test/kernel_composition_test.exs default 3 zero \
  "passed=one page of shipped code starts a runtime creates a session submits a prompt and consumes events" \
...
run_selector 9 apps/loopex_cli/test/coding_task_test.exs default 4 positive \
  ...
  "excluded=one real provider task edits a real repository across several turns and the operator sees the committed result"

run_selector 9 apps/loopex_cli/test/coding_task_test.exs real 1 positive \
  "passed=one real provider task edits a real repository across several turns and the operator sees the committed result" \
...
[ -n "$real_path_record" ] \
  || fail "the real-provider role retained no non-secret real-path identity"
...
validate_negative_demonstrations() {
  local path=docs/evidence/M2-negative-demonstrations.md mechanism
  local mechanisms=(
    committed_history_projection
    tool_definition_generation_binding
    workspace_path_scope_containment
    host_policy_deny_prestart_refusal
    cancellation_cleanup_confirmation
  )
  for mechanism in "${mechanisms[@]}"; do
    grep -qF "\"mechanism_disabled\":\"$mechanism\"" "$path" \
      || fail "the negative demonstration for $mechanism is missing or malformed"
  done
  local count
  count="$(grep -c '"mechanism_disabled"' "$path")"
  [ "$count" -eq 5 ] \
    || fail "the negative demonstrations must be exactly five records, not $count"
}

validate_matrix() {
  local path=docs/evidence/M2-toolchain-matrix.md candidate lane
  ...
  candidate="$(grep -oE '^matrix candidate=[0-9a-f]{40} ' "$path" | head -1 | awk '{print $2}')"
  ...
  for lane in darwin-floor darwin-current linux-current; do
    grep -qE "^capture lane=$lane candidate=$candidate .*verdict=CAPTURE exit=0" "$path" \
      || fail "the retained matrix has no valid $lane capture for candidate $candidate"
  done
  for lane in floor current; do
    grep -qE "^m0 lane=$lane candidate=$candidate .*verdict=GREEN exit=0" "$path" \
      || fail "the retained matrix has no exact-candidate M0 re-proof on the $lane pair"
  done
...
if [ "$role" = "capture" ]; then
  # The retained identity fields are exactly the ones the real role sealed into
  # its authoritative result: everything after that result's digest field. They
  # are never re-derived here from candidate prose.
  real_path_fields="${real_path_record#*digest=sha256:}"
  real_path_fields="${real_path_fields#* }"
  [ "$real_path_fields" != "$real_path_record" ] \
    || fail "the real-provider result carries no sealed identity fields"
  note "capture lane=$capture_lane candidate=$(git rev-parse HEAD) seed=$gate_seed executed=$protected_executed verdict=CAPTURE exit=0 os=$(uname -s | tr '[:upper:]' '[:lower:]') arch=$(uname -m) $real_path_fields"
  exit 0
fi
