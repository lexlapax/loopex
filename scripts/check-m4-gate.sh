...
# The inherited lane is register-derived, so while M3 is not Closed it reports
# UNAVAILABLE rather than pretending the M4 closure base exists.
set -euo pipefail
set +a
set +x
...
[ "${LOOPEX_PROVIDER_API_KEY+x}" != x ] || die 'provider input must use the bounded M4 stdin frame'
...
m4_provider_key=""
export -n m4_provider_key
if ! [ -t 0 ]; then
  frame_header=""
  if [ "$role" = full ]; then
    if IFS= read -r -d '' -n 22 frame_header; then
      [ "$frame_header" = LOOPEX_M4_PROVIDER_V1 ] || die 'malformed provider frame header'
      IFS= read -r -d '' -n 16385 m4_provider_key || die 'unterminated or oversized provider frame'
      [ -n "$m4_provider_key" ] && [ "${#m4_provider_key}" -le 16384 ] || die 'empty or oversized provider frame'
      frame_trailing=""
...
for path in docs/plans/M4.md docs/plans/M4-technical.md docs/plans/M4-gate.md docs/plans/README.md; do
  [ -r "$path" ] || die "M4 artifact is unreadable: $path"
done
...
artifacts=$(awk '
  ...
  END { if (!count) exit 2 }
' docs/plans/M4-gate.md) || die 'Bound Artifacts table is empty or malformed'
for required in scripts/m4-gate-support.exs scripts/m3-gate-support.exs scripts/check-closed-gates.sh scripts/m4-outcomes.exs scripts/check-m4-gate.sh scripts/m4-opening-probe.exs scripts/m1-exunit-runner.exs apps/loopex/test/m1_exunit_runner_test.exs .tool-versions; do
  printf '%s\n' "$artifacts" |
...
support() { env LANG=C.UTF-8 LC_ALL=C.UTF-8 elixir scripts/m4-gate-support.exs --m4-gate-support "$@"; }
support inspect "$root"
if [ "$role" = inspect ]; then
  printf '%s\n' 'M4 inspection OK: scaffold artifact hashes verified; no behavioral or closure evidence'
  exit 0
fi
...
printf 'M4 %s: running the durable-interaction opening witness only\n' "$role"
result=0
opening_result=0
...
case "$result" in
  0)
    [ "$(tail -n 1 "$task_root/probe.log")" = 'M4 opening GREEN: policy defer commits a pending interaction and suspends the run' ] || die 'probe exited 0 without its completed behavioral witness' ;;
  1)
    [ "$(tail -n 1 "$task_root/probe.log")" = 'M4 gate RED: policy defer denies the tool call as interaction_unsupported instead of committing a durable pending interaction' ] || die 'probe exited 1 without its completed behavioral witness'
    if [ "$role" = checkpoint ]; then opening_result=1; else exit 1; fi ;;
  *)
    case "$(tail -n 1 "$task_root/probe.log")" in
      'M4 opening WITNESS ERROR: '*)
        printf '%s\n' 'M4 gate WITNESS ERROR: the opening observation did not establish its named precondition' >&2
        exit 2 ;;
...
if [ "$role" = preflight ]; then
  printf '%s\n' 'M4 preflight OK: opening only; no closure claim'
  exit 0
fi
if [ "$role" = full ]; then
...
if [ "$role" = checkpoint ] && [ -z "$selected_ids" ]; then
  ...
  printf 'M4 checkpoint no outcomes selected: comparison=%s opening_exit=%s; diagnostic only, not closure evidence\n' "$comparison" "$opening_result"
  exit "$opening_result"
fi
...
if [ "$role" = checkpoint ]; then
  final_identity=$(support identity "$root" working) || exit $?
  [ "$source_identity" = "$final_identity" ] || die 'source identity changed during the checkpoint'
  printf 'M4 checkpoint selected outcomes passed: outcomes=%s opening_exit=%s; diagnostic only, not closure evidence\n' "$selected_ids" "$opening_result"
  exit "$opening_result"
fi
